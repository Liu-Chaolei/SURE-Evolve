...
def validate_execution_surface(data: dict[str, Any]) -> list[str]:
    ...
    Specifically checks that the entrypoint does not reference sample/test
    artifacts. Model-local execution surfaces normally live under
    src/sure_eval/models/, so the script-routing-only src/ ban does not apply
    here.
    """
    errors: list[str] = []
    ...
    if entrypoint:
        for forbidden in ("demo/", "examples/", "tests/"):
