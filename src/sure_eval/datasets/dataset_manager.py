...
AISPEECH_SOURCE_ROOT = Path("/hpc_stor08/external_ds/aispeech")


@dataclass(frozen=True)
class DatasetSourceRef:
    """Resolved AiSpeech ds_pool dataset source."""

    entry: str
    source_dataset_root: Path
    annotation_source_root: Path
...
class DatasetManager:
    ...
    def _allowed_aispeech_source_roots(self) -> tuple[Path, ...]:
        configured = os.environ.get("SURE_EVAL_AISPEECH_SOURCE_ROOTS", "")
        roots = [Path(item).expanduser() for item in configured.split(os.pathsep) if item.strip()]
        return tuple(roots or [AISPEECH_SOURCE_ROOT])

    def _lexical_absolute_path(self, path: Path) -> Path:
        return Path(os.path.abspath(os.path.expanduser(str(path))))

    def _version_sort_key(self, version_id: str) -> tuple[Any, ...]:
        return tuple(int(token) if token.isdigit() else token for token in re.split(r"(\d+)", version_id))

    def _is_under_allowed_aispeech_root(self, path: Path) -> bool:
        lexical = self._lexical_absolute_path(path)
        for root in self._allowed_aispeech_source_roots():
            root_lexical = self._lexical_absolute_path(root)
            try:
    ...
    def is_aispeech_source_entry(self, entry: str | Path) -> bool:
        """Return whether an input looks like an allowed AiSpeech source root."""
        try:
            path = Path(str(entry)).expanduser()
        except TypeError:
            return False
        return path.is_absolute() and self._is_under_allowed_aispeech_root(path)

    def resolve_aispeech_source_entry(
        self,
        entry: str | Path,
        version_id: str | None = None,
        annotation_source_root: str | Path | None = None,
    ) -> DatasetSourceRef:
        """Resolve an allowed `/hpc_stor08/.../ds_pool/<dataset>` source root.

        Main-flow dataset identity is derived only from this source root and the
        selected dataset version: `<source_dataset_name>__<version_id>`.
        """

        raw_entry = str(entry)
        source_root = self._lexical_absolute_path(Path(raw_entry))
        if not source_root.is_absolute() or not self._is_under_allowed_aispeech_root(source_root):
            allowed = ", ".join(str(root) for root in self._allowed_aispeech_source_roots())
            raise ValueError(
        ...
        if "ds_pool" not in parts:
            raise ValueError(f"AiSpeech dataset source root must include a ds_pool component: {raw_entry}")
        ds_pool_index = parts.index("ds_pool")
        if len(parts) <= ds_pool_index + 1:
            raise ValueError(f"AiSpeech dataset source root must end with ds_pool/<dataset_name>: {raw_entry}")
        if len(parts) > ds_pool_index + 2:
            raise ValueError(
                "AiSpeech dataset input must be the dataset root, not a nested file or directory: "
                f"{raw_entry}"
            )

        source_dataset_name = parts[ds_pool_index + 1]
        annotation_root = source_root
        if annotation_source_root is not None:
            annotation_root = self._lexical_absolute_path(Path(str(annotation_source_root)))
            if not self._is_under_allowed_aispeech_root(annotation_root):
                raise ValueError(
                    "AiSpeech annotation source root must be under an allowed AiSpeech root: "
                    f"{annotation_source_root}"
                )
            annotation_parts = annotation_root.parts
            if "ds_pool" not in annotation_parts:
                raise ValueError(
                    "AiSpeech annotation source root must include a ds_pool component: "
                    f"{annotation_source_root}"
                )
            annotation_ds_pool_index = annotation_parts.index("ds_pool")
            if len(annotation_parts) != annotation_ds_pool_index + 2:
                raise ValueError(
                    "AiSpeech annotation source root must end with ds_pool/<dataset_name>: "
                    f"{annotation_source_root}"
                )

        sample_files_dir = annotation_root / "sample_files"
        if not sample_files_dir.exists():
            raise FileNotFoundError(f"AiSpeech sample_files directory not found: {sample_files_dir}")

        if version_id:
            selected_version = version_id
            version_dir = sample_files_dir / selected_version
            if not version_dir.exists():
                raise FileNotFoundError(f"AiSpeech dataset version not found: {version_dir}")
        else:
            versions = sorted(
                (
                    path.name
                    for path in sample_files_dir.iterdir()
                    if path.is_dir() and (path / "sample.jsonl").exists()
                ),
                key=self._version_sort_key,
            )
            if not versions:
                raise FileNotFoundError(f"No sample_files/<version>/sample.jsonl found under {sample_files_dir}")
            selected_version = versions[-1]
            if len(versions) > 1:
                logger.info(
                    "Selected latest AiSpeech dataset version",
                    source_root=raw_entry,
                    versions=versions,
        ...
        if not sample_jsonl.exists():
            raise FileNotFoundError(f"AiSpeech sample.jsonl not found: {sample_jsonl}")
        if not ds_jsonl.exists():
            raise FileNotFoundError(f"AiSpeech ds.jsonl not found: {ds_jsonl}")
        if not raw_dir.exists():
            raise FileNotFoundError(f"AiSpeech raw audio directory not found: {raw_dir}")

        if annotation_root != source_root:
            annotation_basenames: set[str] = set()
            with sample_jsonl.open("r", encoding="utf-8") as handle:
                for line_no, line in enumerate(handle, start=1):
                    ...
                    try:
                        ...
                        raise ValueError(
                            f"Invalid annotation JSON at {sample_jsonl}:{line_no}: {exc.msg}"
                        ) from exc
                    raw_path = str((record.get("attribute") or {}).get("path") or "")
                    if not raw_path:
                        raise ValueError(
                            f"Annotation record has no attribute.path at {sample_jsonl}:{line_no}"
                        )
                    annotation_basenames.add(Path(raw_path).name)

            source_basenames = {path.name for path in raw_dir.iterdir() if path.is_file()}
            if annotation_basenames != source_basenames:
                missing = sorted(annotation_basenames - source_basenames)
                extra = sorted(source_basenames - annotation_basenames)
                raise ValueError(
                    "AiSpeech annotation and source audio basename sets differ: "
        ...
        return DatasetSourceRef(
            ...
            annotation_source_root=annotation_root,
    ...
    def _first_jsonl_row(self, path: Path) -> dict[str, Any]:
        if not path.exists():
            return {}
        with path.open("r", encoding="utf-8") as handle:
            for line in handle:
    ...
    def get_source_info(self, dataset_name: str) -> dict[str, Any]:
        """Return stable source identity for a prepared dataset when available."""

        if self.is_aispeech_source_entry(dataset_name):
            source_ref = self.resolve_aispeech_source_entry(dataset_name)
            return {
                "source": "aispeech_ds_pool",
                "dataset_id": source_ref.dataset_id,
                "source_dataset_root": str(source_ref.source_dataset_root),
                "source_dataset_name": source_ref.source_dataset_name,
        ...
        first_row = self._first_jsonl_row(jsonl_path)
        metadata = first_row.get("metadata") if isinstance(first_row.get("metadata"), dict) else {}

        if metadata.get("source") == "aispeech_ds_pool":
            return {
                "source": "aispeech_ds_pool",
                "dataset_id": canonical_name,
                "source_dataset_root": metadata.get("source_dataset_root"),
                "source_dataset_name": metadata.get("source_dataset_name"),
                "version_id": metadata.get("version_id"),
                "naming_policy": "source_dataset_name__version_id",
            }

        source_json = self.sure_dir / canonical_name / "oref" / "source.json"
        source_payload = self._load_single_json_object(source_json)
        if source_payload.get("source") == "aispeech_ds_pool":
            return {
                "source": "aispeech_ds_pool",
                "dataset_id": canonical_name,
                "source_dataset_root": source_payload.get("source_dataset_root"),
                "source_dataset_name": source_payload.get("source_dataset_name"),
                "version_id": source_payload.get("version_id"),
                "naming_policy": "source_dataset_name__version_id",
            }

        if metadata.get("source") == "oref_platform":
            return {
                "source": "oref_platform",
                "dataset_id": canonical_name,
                "source_dataset_name": metadata.get("oref_dataset"),
                "oref_dataset": metadata.get("oref_dataset"),
                "version_id": metadata.get("version_id"),
                "naming_policy": "legacy_config_name__version_id__task",
            }

        projection_meta = self._oref_platform_meta_for_projection(canonical_name)
        if projection_meta:
            oref_name, meta = projection_meta
            dataset_root = self._resolve_config_path(meta["dataset_root"]) if meta.get("dataset_root") else None
            return {
                "source": "oref_platform",
                "dataset_id": canonical_name,
                "source_dataset_root": str(dataset_root) if dataset_root else None,
                "source_dataset_name": meta.get("platform_dataset_name") or oref_name,
                "oref_dataset": meta.get("platform_dataset_name") or oref_name,
    ...
    def get_jsonl_path(self, dataset_name: str) -> Path:
        ...
        if self.is_aispeech_source_entry(dataset_name):
            source_ref = self.resolve_aispeech_source_entry(dataset_name)
            return self.jsonl_dir / f"{source_ref.dataset_id}.jsonl"
        canonical_name = self._canonical_name(dataset_name)
    ...
    def download_and_convert(self, dataset_name: str) -> Path:
        ...
        """
        if self.is_aispeech_source_entry(dataset_name):
            return self.convert_aispeech_source_to_jsonl(self.resolve_aispeech_source_entry(dataset_name))
    ...
    def _infer_aispeech_task_language(
        self,
        *,
        ds_meta: dict[str, Any],
        sample_jsonl_path: Path,
    ) -> tuple[str, str]:
        supported_tasks = ds_meta.get("supported_tasks")
        if isinstance(supported_tasks, list) and supported_tasks:
            task = str(supported_tasks[0]).upper()
        else:
            task = str(ds_meta.get("task") or "ASR").upper()

        language = str((((ds_meta.get("audio") or {}).get("speech") or {}).get("language")) or "")
        if not language:
            with sample_jsonl_path.open("r", encoding="utf-8") as handle:
                for line in handle:
                    ...
                    try:
                        ...
                        continue
                    annotations = sample.get("annotation") or []
                    for annotation in annotations:
                        if not isinstance(annotation, dict):
                            continue
                        transcription = annotation.get("transcription") or {}
                    ...
                    if language:
                        break
        return task, language or "auto"

    def convert_aispeech_source_to_jsonl(self, source_ref: DatasetSourceRef | str | Path) -> Path:
        """Project an allowed AiSpeech source root into a canonical SURE JSONL."""

        if not isinstance(source_ref, DatasetSourceRef):
            source_ref = self.resolve_aispeech_source_entry(source_ref)

        jsonl_path = self.jsonl_dir / f"{source_ref.dataset_id}.jsonl"
        package_dir = self.sure_dir / source_ref.dataset_id
        projection_dir = package_dir / "projections" / "asr_transcription_v1"

        if jsonl_path.exists():
            logger.info("Using existing AiSpeech source projection", dataset=source_ref.dataset_id, jsonl=str(jsonl_path))
            return jsonl_path

        ds_meta = self._load_single_json_object(source_ref.ds_jsonl)
        task, language = self._infer_aispeech_task_language(
            ds_meta=ds_meta,
            sample_jsonl_path=source_ref.sample_jsonl,
        )
        if task != "ASR":
            raise NotImplementedError(f"AiSpeech source projection currently supports ASR only, got: {task}")

        rows: list[dict[str, Any]] = []
        skipped: list[dict[str, Any]] = []
        source_records = 0
        seen_keys: set[str] = set()
        with source_ref.sample_jsonl.open("r", encoding="utf-8") as handle:
            for line_no, line in enumerate(handle, start=1):
                line = line.strip()
                if not line:
                    continue
                ...
                if not raw_path:
                    ...
                    continue
                if source_ref.annotation_source_root != source_ref.source_dataset_root:
                    audio_path = source_ref.raw_dir / Path(raw_path).name
                else:
                    audio_path = self._resolve_oref_audio_path(raw_path, source_ref.raw_dir)
                ...
                if attr.get("size") is not None and int(attr["size"]) != audio_path.stat().st_size:
                    skipped.append(
                        {
                            "line": line_no,
                            "reason": f"audio_size_mismatch: expected {attr['size']} got {audio_path.stat().st_size}",
                        }
                    )
                    continue
                ...
                rows.append(
                    {
                        ...
                        "task": task,
                        "language": language,
                        "dataset": source_ref.dataset_id,
                        "sample_rate": attr.get("sample_rate"),
                        "duration_ms": attr.get("duration", 0),
                        "metadata": {
                            "source": "aispeech_ds_pool",
                            "source_dataset_root": str(source_ref.source_dataset_root),
                            "annotation_source_root": str(source_ref.annotation_source_root),
                            "source_dataset_name": source_ref.source_dataset_name,
                            "version_id": source_ref.version_id,
                            "sample_id": record.get("sample_id"),
        ...
        if skipped:
            ...
            raise ValueError(
                f"AiSpeech source conversion skipped {len(skipped)} of {source_records} records for "
                f"{source_ref.source_dataset_root}; first issues: {reasons}"
            )
        if not rows:
            raise ValueError(f"AiSpeech source conversion produced no samples for {source_ref.source_dataset_root}")
        ...
        source_payload = {
            ...
            "source_dataset_name": source_ref.source_dataset_name,
            "version_id": source_ref.version_id,
            "raw_dir": str(source_ref.raw_dir),
            "ds_jsonl": str(source_ref.ds_jsonl),
            "sample_jsonl": str(source_ref.sample_jsonl),
            "created_at": _utc_now(),
        }
        self._copy_oref_source_files(
            package_dir=package_dir,
            ds_jsonl_path=source_ref.ds_jsonl,
            sample_jsonl_path=source_ref.sample_jsonl,
        ...
        mapping = {
            ...
            "fields": {
                ...
                "language": "ds.audio.speech.language | annotation.transcription.language",
        ...
        conversion_report = {
            ...
            "source_dataset_root": str(source_ref.source_dataset_root),
            "annotation_source_root": str(source_ref.annotation_source_root),
            "source_sample_jsonl": str(source_ref.sample_jsonl),
            "source_ds_jsonl": str(source_ref.ds_jsonl),
        ...
        logger.info(
            "Converted AiSpeech source dataset",
            dataset=source_ref.dataset_id,
            source_root=str(source_ref.source_dataset_root),
            samples=len(rows),
    ...
    def get_info(self, dataset_name: str) -> dict[str, Any] | None:
        ...
        if self.is_aispeech_source_entry(dataset_name):
            canonical_name = self.resolve_aispeech_source_entry(dataset_name).dataset_id
        else:
            canonical_name = self._canonical_name(dataset_name)
        jsonl_path = self.jsonl_dir / f"{canonical_name}.jsonl"
        source_info = self.get_source_info(canonical_name)
        if source_info.get("source") == "aispeech_ds_pool":
            first_row = self._first_jsonl_row(jsonl_path)
            return {
                "name": canonical_name,
                ...
                "dataset_id": canonical_name,
                "task": first_row.get("task"),
                "language": first_row.get("language"),
                "source": "aispeech_ds_pool",
                "source_dataset_root": source_info.get("source_dataset_root"),
                "source_dataset_name": source_info.get("source_dataset_name"),
                "version_id": source_info.get("version_id"),
                "jsonl_path": str(jsonl_path),
                "num_samples": self._count_jsonl_rows(jsonl_path),
                "is_available": jsonl_path.exists(),
                "naming_policy": "source_dataset_name__version_id",
            }
