...
from sure_eval.agent.trigger_vc import resolve_required_partition
from sure_eval.core.config import Config
from sure_eval.core.logging import configure_logging, get_logger
from sure_eval.datasets import DatasetManager
from sure_eval.evaluation.cli import agent_app, env_app, metric_app
from sure_eval.evaluation.env_check import doctor_payload
...
def get_evaluator(config_path: Optional[str] = None):
    """Load the legacy evaluator only for legacy CLI commands."""
...
def list_metrics() -> None:
    ...
    metrics = {
        "ASR": ["cer", "wer"],
        "S2TT": ["bleu", "bleu_char", "chrf", "xcomet_xl", "bleurt_20"],
        "SER": ["accuracy"],
        "GR": ["accuracy"],
        "SLU": ["accuracy"],
        "SD": ["der"],
...
def doctor(
    ...
    model_name: Annotated[
        Optional[str],
        typer.Argument(help="Optional model name for model-runtime diagnostics"),
    ] = None,
    json_output: Annotated[
        bool,
        typer.Option("--json", help="Print JSON; with a model, an extra path writes the report"),
    ] = False,
    verbose: Annotated[
        bool,
        typer.Option("--verbose", "-v", help="Show all evaluation environment checks"),
    ] = False,
) -> None:
    """Run diagnostics for evaluation pipelines or a model runtime."""

    extra_args = list(ctx.args)
    if len(extra_args) > 1:
        console.print(f"[bold red]Error:[/bold red] Unexpected doctor arguments: {' '.join(extra_args)}")
        raise typer.Exit(2)

    if model_name is None:
        payload = doctor_payload()
        if extra_args:
            console.print(f"[bold red]Error:[/bold red] Unexpected doctor argument: {extra_args[0]}")
            raise typer.Exit(2)
        if json_output:
            sys.stdout.write(json.dumps(payload, ensure_ascii=False) + "\n")
        else:
            table = Table(title="SURE-EVAL Doctor")
    ...
    model = get_model_registry().get_model(model_name)
    readiness = get_runtime_readiness(model, model_name=model_name)
    json_report = Path(extra_args[0]) if json_output and extra_args else None
    ...
    if json_report is not None:
        ...
        sys.stdout.write(json.dumps(readiness, ensure_ascii=False) + "\n")
    else:
        table = Table(title=f"Model Runtime Doctor: {model_name}")
        table.add_column("Check", style="cyan")
        table.add_column("Status", style="white")
        table.add_column("Details", style="white")
        for check in readiness["checks"]:
            table.add_row(
                str(check["name"]).replace("_", " ").title(),
                "PASS" if check["passed"] else "FAIL",
                str(check["details"]),
            )
        console.print(table)
        if readiness["action_hint"]:
            console.print(f"[yellow]Hint:[/yellow] {readiness['action_hint']}")
...
def submit_run(
    ...
    partition: Optional[str] = typer.Option(None, "--partition", "-p", help="Required GPU partition (defaults to pdgpu-4090 if omitted)"),
    ...
    Auto-selects image and memory, and requires the requested partition.
    If no partition is provided, pdgpu-4090 is required. No partition fallback
    is attempted.
    ...
    surface_partition: Optional[str] = None
    if run_dir:
        ...
        try:
            ...
            surface_partition = resolve_required_partition(surface)
        except Exception as exc:
            console.print(f"[bold red]Error:[/bold red] failed to read execution surface: {exc}")
            raise typer.Exit(1) from exc

    resolved_partition = partition or surface_partition
