...
CONFIG_PATH="${SURE_MASTER_CONFIG:-configs/sure_master/gpt-5-f5tts-staged-axes-mixed.yaml}"
...
if [[ -n "${SURE_MASTER_LOG_FILE:-}" ]]; then
  ...
  LOG_FILE="${LOG_DIR}/sure_master_f5tts_${RUN_NAME}.log"
else
  LOG_FILE="${LOG_DIR}/sure_master_f5tts_mixed_local_$(date +%Y%m%d_%H%M%S).log"
fi
...
if ! command -v vc >/dev/null 2>&1; then
  echo "vc command is required because all candidate execution runs as VC child jobs."
  exit 2
fi
