...
def test_main_flow_shell_propagates_explicit_annotation_source_overlay() -> None:
    template_path = Path("docs/agents/main_flow_agent/templates/run_single_model.sh")
    if not template_path.exists():
        pytest.skip("main-flow templates are not part of the standalone evaluation package")
    template = template_path.read_text(encoding="utf-8")

    assert 'ANNOTATION_SOURCE_ROOT="${ANNOTATION_SOURCE_ROOT:-}"' in template
    assert "--annotation-source-root" in template
    assert "ANNOTATION_SOURCE_ROOT requires exactly one dataset" in template

    surface = json.loads(
        Path("docs/agents/main_flow_agent/templates/main_agent_execution_surface.json").read_text(
            encoding="utf-8"
        )
    )
    dataset_source = surface["resolved_inputs"]["dataset_sources"][0]
    assert "annotation_source_root" in dataset_source
