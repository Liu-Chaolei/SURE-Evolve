...
def test_predict_dry_run_passes_for_qwen_qwen3_asr_1_7b(tmp_path: Path) -> None:
    runner = CliRunner()
...
def test_resolve_runtime_command_uses_model_local_paths() -> None:
    model = ModelRegistry().get_model("Qwen__Qwen3-ASR-1.7B")
    assert model is not None

    command, working_dir, env = _resolve_runtime_command(model)

    assert command[0].endswith("src/sure_eval/models/Qwen__Qwen3-ASR-1.7B/.venv/bin/python")
...
def test_model_language_registry_maps_qwen3_and_default() -> None:
    assert map_language_for_model(model_name="Qwen__Qwen3-ASR-1.7B", language="en") == "English"
