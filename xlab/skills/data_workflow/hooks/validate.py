...
def find_artifact(context: dict[str, Any], artifact_type: str) -> dict[str, Any] | None:
...
def load_artifact_json(context: dict[str, Any], artifact_type: str) -> tuple[dict[str, Any] | None, Path | None, str | None]:
    artifact = find_artifact(context, artifact_type)
    if not artifact:
...
def read_workflow(context: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if not workflow_path.is_file():
        return None
    value, error = read_json_file(workflow_path)
...
def lifecycle(context: dict[str, Any], label: str) -> dict[str, Any]:
    ...
    if point == "pre_start":
        return {"ok": True, "state_patch": {"phase": {"id": "workflow", "label": f"{label} ready", "status": "running", "progress": 0}, "message": f"Workflow stages: {' → '.join(chain)}.", "checkpoint": {"id": "workflow", "label": f"{label} lifecycle", "resumable": True, "data": {"stages": chain}}, "next_actions": ["Use xlab_stage at every declared stage boundary.", "Complete stages with typed artifact arrays before finishing."]}}
    if point == "pre_stage":
    ...
    if point == "on_resume":
        return {"ok": True, "state_patch": {"phase": {"id": "resume", "label": f"{label} resume point", "status": "running"}, "counters": count_statuses(workflow), "checkpoint": {"id": "workflow", "label": "Workflow resume state", "resumable": True, "data": {"stages": workflow_stages(workflow)}}, "next_actions": ["Resume the next pending or blocked workflow stage."]}}
    if point == "on_cancel":
...
def blocked(message: str, repair: str) -> dict[str, Any]:
    return {"ok": False, "message": message, "repair": repair, "state_patch": {"phase": {"id": "validate", "label": "Dataset validation gate", "status": "blocked"}, "diagnostics": [{"severity": "error", "code": "dataset_gate", "message": message, "repair": repair}], "next_actions": [repair]}}
...
def validate_finish(context: dict[str, Any]) -> dict[str, Any]:
    ...
    contract, path, error = load_artifact_json(context, "dataset")
    if error:
        return blocked(error, "Create a dataset artifact and rerun data_validate before finishing.")
    assert contract is not None
    if finish.get("status") == "success" and (
        contract.get("valid") is not True
        or contract.get("rows", 0) < 1
        or contract.get("duplicates") != 0
        or contract.get("errors")
    ):
        return blocked("Dataset did not pass row, duplicate, and parsing validation.", "Repair the dataset and rerun data_validate.")
    return {"ok": True, "state_patch": {"phase": {"id": "validate", "label": "Dataset validated", "status": "success", "progress": 1}, "counters": {"rows": int(contract.get("rows", 0) or 0), "duplicates": int(contract.get("duplicates", 0) or 0)}, "artifacts": [{"type": "dataset", "name": "dataset", "path": str(path), "status": "ready"}]}}


def dispatch(context: dict[str, Any]) -> dict[str, Any]:
    if str(context.get("point") or "pre_finish") != "pre_finish":
        return lifecycle(context, "Data workflow")
...
def main() -> None:
    try:
        ...
        emit({"ok": False, "message": f"Data workflow hook failed: {error}", "repair": "Fix hooks/validate.py and rerun the hook."})
