...
function packagePaths(ctx: XlabHookContext): {
  script: string;
  mineruManifest: string;
  structuresManifest: string;
...
function eventManifestPath(ctx: XlabHookContext): string | undefined {
  ...
  return typeof finishPath === "string" && finishPath.trim() !== ""
    ? resolveProjectOrRunPath(ctx, finishPath)
    : undefined;
}
...
interface PackagedCliCommand extends ParsedCommand {
  phase: "mineru" | "structure" | "extract" | "build" | "audit" | "run" | "resume";
}

const SHELL_CONTROL_PATTERN = /(?:[;&|<>`]|\$\(|\n|\r)/;
const ALLOWED_CLI_PHASES = new Set([
  "mineru",
  "structure",
  "extract",
  "build",
...
function cliPhase(
  command: string,
): "mineru" | "structure" | "extract" | "build" | "audit" | "run" | "other" {
...
function packagedCliRejection(
  ...
  if (!parsed) {
    return {
      repair:
        "Run build_graph.py as one simple command without shell control operators or redirects.",
  ...
  if (!script || !isPackagedScriptPath(script.script, ctx)) {
    return {
      ...
      message:
        "Blocked build_graph.py invocation outside the active skill package.",
    };
  }
  const phase = script.args[0];
  if (!ALLOWED_CLI_PHASES.has(phase)) {
    return {
      repair:
        "Use only build_graph.py mineru, structure, extract, build, audit, run, or resume in the slash-command runtime.",
  ...
  if (!runDirValuesAreExpected(optionValues(script.args.slice(1), "--run-dir"), ctx)) {
    return {
      ...
      message:
        "Blocked knowledge_graph CLI invocation for a different run directory.",
    };
  }
  return undefined;
}
...
export function preStart(ctx: XlabHookContext): XlabHookResult {
  ...
  try {
    ...
    return failure(
      ...
      `Knowledge graph initialization produced invalid state: ${
        error instanceof Error ? error.message : String(error)
      }`,
    );
  }
  const mineru = isRecord(resources.mineru) ? resources.mineru : {};
  const selectedGpus = Array.isArray(mineru.selected_gpus)
    ? mineru.selected_gpus
    : [];
  const selectedNpus = Array.isArray(mineru.selected_npus) ? mineru.selected_npus : [];
  const resourceSummary =
    selectedNpus.length > 0
      ? `${String(mineru.backend)} on NPU ${selectedNpus.join(", ")}`
      : selectedGpus.length > 0
      ? `${String(mineru.backend)} on GPU ${selectedGpus.join(", ")}`
      : `${String(mineru.backend)} on CPU (${String(mineru.workers)} workers)`;
  return {
    ok: true,
    state_patch: {
      phase: {
      ...
      checkpoint: {
        ...
        resumable: true,
        resume_hint:
          "Rerun the interrupted stage; MinerU, Step 1, and LLM pass checkpoints are checksum-validated.",
      },
    },
  };
}
...
export function preToolCall(ctx: XlabHookContext): XlabHookResult {
  ...
  if (
    /Xcientist-2|XAgora|XForge|PaperGraph|Method-Evolution-Graph/i.test(command)
  ) {
    return failure(
  ...
  if (
    ...
    /(?:artifacts\/(?:mineru\.manifest\.json|paper_structures\.manifest\.json|extractions\.manifest\.json|method_graph\.json|graph\.db|graph_report\.json)|manifest\.json)/.test(
      command,
    )
  ) {
    return failure(
    ...
    if (legalIncomplete(ctx, event)) {
      ...
      return {
        ...
        state_patch: {
          ...
          message:
            "The packaged graph runtime completed and wrote an incomplete manifest; this is a recoverable quality-gate outcome, not a crash.",
        ...
        diagnostics: [
          {
            ...
            repair:
              "Inspect the packaged CLI error and rerun the same stage; completed per-paper checkpoints are preserved.",
          },
        ],
      },
    };
...
export function preFinish(ctx: XlabHookContext): XlabHookResult {
  ...
  if (
    audit.error ||
    (audit.status !== 0 && audit.status !== 2) ||
    !existsSync(paths.report)
  ) {
    return failure(
      "Repair the reported artifact and rerun build_graph.py audit.",
      `Knowledge graph audit could not run: ${
        audit.error?.message || audit.stderr.trim() || `exit ${audit.status}`
      }`,
    );
  }
...
export function onResume(ctx: XlabHookContext): XlabHookResult {
  ...
  try {
    report = existsSync(packagePaths(ctx).report)
      ? readJson(packagePaths(ctx).report)
      : undefined;
  } catch (error) {
    readDiagnostics.push({
      ...
      repair:
        "Rerun build_graph.py audit to regenerate artifacts/graph_report.json.",
    });
  }
  const counts = isRecord(report?.counts) ? report.counts : {};
  const counters = {
  ...
  return {
    ...
    state_patch: {
      phase: {
        id: "resume",
        label: passed
          ? "Knowledge graph ready to finish"
          : blockers.length > 0
            ? "Knowledge graph blocked at quality gate"
            : "Resuming knowledge graph",
        status: passed
          ? "success"
          : blockers.length > 0
            ? "incomplete"
      ...
      message:
        "MinerU bundles, PaperGraph Step 1 structures, and validated per-pass LLM checkpoints will be reused.",
      counters,
      diagnostics: [
        ...readDiagnostics,
        ...(report
      ...
      checkpoint: {
        ...
        label: existsSync(packagePaths(ctx).extractionsManifest)
          ? "Extraction manifest present"
          : existsSync(packagePaths(ctx).structuresManifest)
            ? "Structure manifest present"
            : existsSync(packagePaths(ctx).mineruManifest)
              ? "MinerU manifest present"
              : "No reusable stage manifest",
        resumable: true,
        resume_hint: passed
          ? "Run build_graph.py audit and finish with status success."
          : "Rerun the interrupted mineru, structure, extract, build, or audit stage.",
      },
      next_actions: passed
        ? ["Run build_graph.py audit", "Call xlab_finish with status success"]
        : blockers.length > 0
          ? [
...
export function onCancel(): XlabHookResult {
  return {
    ...
    state_patch: {
      ...
      message:
        "Completed MinerU, structure, and LLM checkpoints were preserved.",
    },
  };
}
  ...
  return {
    ...
    state_patch: {
      ...
      message:
        ...
        ctx.run.lastRepair ??
        "Knowledge graph stopped before completion.",
      checkpoint: {
