...
def get_ranked_gpu_candidates() -> List[Tuple[str, int]]:
    ranked = _gpu_candidates_from_torch() or _gpu_candidates_from_nvidia_smi()
    ranked.sort(key=lambda item: item[1], reverse=True)
    return ranked
...
def load_sentence_transformer_auto(model_name: str, logger=None) -> tuple[SentenceTransformer, str]:
    if fake_llm_enabled():
        if logger is not None:
            logger.info("Using deterministic fake SentenceTransformer for literature_survey smoke tests")
        return FakeSentenceTransformer(model_name), "cpu"
...
class FakeSentenceTransformer:
    """Deterministic local embedding shim for offline SurveyAgent smoke tests."""
    ...
    def encode(
        ...
        **_kwargs: Any,
        ...
        values = [sentences] if isinstance(sentences, str) else list(sentences or [])
        vectors = torch.stack([self._embed(str(value)) for value in values]) if values else torch.empty((0, self.embedding_dim))
...
def fake_llm_enabled() -> bool:
    return os.environ.get("XLAB_LITERATURE_SURVEY_FAKE_LLM", "").strip().lower() in {"1", "true", "yes", "on"}
