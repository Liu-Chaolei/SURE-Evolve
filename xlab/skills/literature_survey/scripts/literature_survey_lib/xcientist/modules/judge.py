...
class Judge():
    ...
    def citation_quality(self, survey_with_reference, references):
        ...
        scores = [0] * len(claims)
        workers = int(self.config.APIInfo.batch_chat_agent_worker)
        with ThreadPoolExecutor(max_workers=workers) as executor:
            futures = []
            for i in range(len(claims)):
                keynotes = [index_to_keynotes[index] for index in sources_ids[i] if index not in fail_index]
                futures.append(executor.submit(self.__nli, keynotes, claims[i], scores, i))
            for future in tqdm(futures, desc="Citation support evaluation"):
                future.result()
        citation_num = 0
            ...
            for j, claim, source_ids in zip(range(len(claims)), claims, sources_ids):
                ...
                if scores[j] == 1:
                    for index in source_ids:
                        ...
                        com_keynotes = [index_to_keynotes[other] for other in source_ids if other != index and other not in fail_index]
                        futures.append(executor.submit(self.__relevant, keynotes, com_keynotes, claim, precisions, j))
            for future in tqdm(futures, desc="Citation relevance evaluation"):
                future.result()
