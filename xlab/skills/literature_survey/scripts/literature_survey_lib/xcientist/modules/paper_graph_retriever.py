...
def hydra_main(*args, **kwargs):
    ...
    def decorator(func):
        def wrapped(*call_args, **call_kwargs):
            raise RuntimeError("hydra is required to run PaperGraphRetriever as a standalone CLI.")
...
class PaperGraphRetriever:
    def __init__(self, config, data_manager = None):
        ...
        cache_path = getattr(
            config.BasicInfo,
            'cache_path',
            os.path.join(os.environ.get("XLAB_SURVEY_AGENT_BASE_DIR", "."), "paper_graph_cache"),
        )
...
@hydra_main(config_path=str(_PACKAGE_CONFIG_DIR), config_name="deep_survey_batch_others_huoshan", version_base=None)
def main(config):
