...
import torch
import gc
from .pe import PAPER_RELATEDNESS_BASED_ON_TITLE_AND_ABSTRACT, SEED_PAPER_SELECTION
import diskcache as dc
from ..utils.utils import get_hash, extract_json
from ..utils.gpu_utils import load_sentence_transformer_auto
try:
    import hydra
except ImportError:  # optional; only needed for this module's standalone debug CLI.
    hydra = None
from pathlib import Path
_PACKAGE_CONFIG_DIR = Path(__file__).resolve().parents[1] / "config"


def hydra_main(*args, **kwargs):
    if hydra is not None:
        return hydra.main(*args, **kwargs)

    def decorator(func):
        def wrapped(*call_args, **call_kwargs):
            raise RuntimeError("hydra is required to run WorkCollector as a standalone CLI.")
...
class WorkCollector:
    ...
    def filter_papers_local_paper_graph(self, expanded_papers: List[str], seed_paper_ids: List[str]):
        ...
        if self.advanced_filter_in_local_paper_graph_expansion:
            ...
            related_embeddings = None
            if related_pids:
                related_embeddings = model.encode(
                    [paper2text(pid) for pid in related_pids],
                    convert_to_tensor=True,
                    batch_size=self.config.ModuleInfo.WorkCollector.sentence_transformer_batch_size,
                    show_progress_bar=True,
                )
...
@hydra_main(config_path=str(_PACKAGE_CONFIG_DIR), config_name="deep_survey_batch_xiaomi_fast", version_base=None)
def main(config):
