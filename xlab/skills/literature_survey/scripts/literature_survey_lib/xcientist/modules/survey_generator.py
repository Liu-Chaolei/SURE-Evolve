...
from ...common import atomic_write_json, atomic_write_text, read_json
...
class SurveyGenerator:
    ...
    def generate_outline_in_steps(
        ...
        if not isinstance(outline, dict) or not outline.get("sections"):
            raise ValueError("SurveyAgent outline generation did not return any sections.")
    ...
    def generate_outline_draft_outline(
        ...
        for batch_idx in range(num_batches):
            ...
            if not valid:
                message = f"SurveyAgent outline draft generation failed for batch {batch_idx + 1} after {max_retry_in_loop} attempt(s)."
    ...
    def _validate_assignment(self, result, info_dict = None):
        ...
        err_papers = []
        if not isinstance(papers_assignment, list) or not all(isinstance(paper, dict) for paper in papers_assignment):
            raise ValueError("Paper assignment must be a list of records")
        aliases = {}
        for canonical in papers:
            suffix = canonical.partition(":")[2]
            if suffix:
                aliases.setdefault(suffix, set()).add(canonical)
        for paper in papers_assignment:
            paper_id = paper.get("paper_id")
            matches = aliases.get(paper_id, set())
            if paper_id not in papers and len(matches) == 1:
                paper["paper_id"] = next(iter(matches))
        batches = info_dict.get("batches")
        if batches:
            expected = set(batches[info_dict.get("idx", 0)])
            returned = [paper.get("paper_id") for paper in papers_assignment]
            if not expected.issubset(returned) or len(returned) != len(set(returned)):
                raise ValueError(f"Paper assignment omitted batch papers or repeated paper IDs; missing IDs: {sorted(expected - set(returned))}")
        outline = info_dict.get("outline")
        if outline:
            targets = {section["title"]: {sub["title"] for sub in section.get("subsections", [])} for section in outline["sections"]}
            for paper in papers_assignment:
                assignment = paper.get("assignment")
                if not isinstance(assignment, dict):
                    raise ValueError("Missing assignment object")
                if not assignment and not str(paper.get("exclusion_reason") or "").strip():
                    raise ValueError("Unassigned papers require an explicit exclusion reason")
                for section, subsections in list(assignment.items()):
                    parents = [parent for parent, children in targets.items() if section in children]
                    if section not in targets and len(parents) == 1 and subsections == [section]:
                        parent = parents[0]
                        if isinstance(assignment.get(parent, []), list):
                            assignment.pop(section)
                            assignment[parent] = list(dict.fromkeys([*assignment.get(parent, []), section]))
                for section, subsections in assignment.items():
                    if section not in targets or not isinstance(subsections, list) or not set(subsections).issubset(targets[section]):
                        raise ValueError(f"Assignment refers to an unknown section or subsection: {section!r} -> {subsections!r}")
    ...
    def generate_outline_assign_papers(
        ...
        for batch_idx in range(num_batches):
            ...
            prompt += "\nReturn one record for EVERY paper in the new paper keynotes. If a paper is outside the survey scope, return assignment={} and an explicit exclusion_reason. Never silently skip a paper."
            prompts.append(prompt)
            assignment_batches.append(batch_papers)
        ...
        if self.outline_fast_mode:
            ...
            for result in results:
                papers_assignment = extract_json(result)
                outline_with_paper_assignment = self._assign_paper(papers_assignment, outline_with_paper_assignment, omit_err = omit_err)
            outline_with_paper_assignment["assignment_reviewed_papers"] = sorted({paper["paper_id"] for result in results for paper in extract_json(result)})
            outline_with_paper_assignment["excluded_papers"] = [paper for result in results for paper in extract_json(result) if not paper.get("assignment")]
        outline_with_paper_assignment["assignment_version"] = 2
        return outline_with_paper_assignment
    ...
    def _apply_revision_to_text(self, original_text: str, revision_json: dict) -> str:
        ...
        if idx == -1:
            ...
            # TODO: fuzy match?
            return original_text
        if original_text.count(old_str) != 1:
            self.logger.warning("Revision target is ambiguous; preserving the original text.")
            return original_text
        
        new_text = original_text[:idx] + new_str + original_text[idx + len(old_str):]
        if re.findall(r"(?m)^\s*#{1,6}\s+.*$", new_text) != re.findall(r"(?m)^\s*#{1,6}\s+.*$", original_text):
            self.logger.warning("Revision would change section structure; preserving the original text.")
            return original_text
        if self.config.BasicInfo.debug:
    ...
    def review_and_revise_survey_in_parts(self, draft, outline):
        ...
        cache_dir = Path(self.config.BasicInfo.base_dir) / "state/xcientist/reviewed_sections"
        ...
        def review_one(idx):
            ...
            signature = json.dumps({"version": 2, "text": section_text, "previous": previous_section_text,
                                    "next": next_section_text, "outline": outline['sections'][idx],
                                    "topic": self.config.BasicInfo.topic, "model": self.config.APIInfo.llm_model_name,
                                    "endpoint": self.config.APIInfo.llm_api_base_url}, sort_keys=True)
            path = cache_dir / (hashlib.sha256(signature.encode()).hexdigest() + ".json")
            if path.exists():
            ...
            if not isinstance(revised_text, str) or not revised_text.strip():
                raise ValueError(f"Review returned an empty section: {idx}")
            atomic_write_json(path, {"text": revised_text, "review_version": 2})
            return revised_text

        with ThreadPoolExecutor(max_workers=min(len(sections), self.chat_agent.batch_workers)) as executor:
            revised_sections = list(executor.map(review_one, range(len(sections))))
    ...
    def refine_draft(self, draft):
        draft_signature = hashlib.sha256(json.dumps(draft, sort_keys=True).encode()).hexdigest()
        ...
        if self.refine_in_parts:
            ...
            state = Path(self.config.BasicInfo.base_dir) / "state/xcientist"
            atomic_write_text(state / "refined.titled.md", survey)
            atomic_write_json(state / "refined.titled.meta.json", {"draft_sha256": draft_signature, "model": self.config.APIInfo.llm_model_name})
            return self.finalize_citations(survey, draft["outline"])
        ...
        return self.append_bibliography(survey, references)

    def finalize_citations(self, survey, outline=None):
        headings = {}
        for section in (outline or {}).get("sections", []):
            for item in [section, *section.get("subsections", [])]:
                headings[" ".join(item["title"].casefold().split())] = item["title"]

        def internal_reference(match):
            key = " ".join(match.group(1).casefold().split())
            if key in headings and key not in self.database._title_index and key not in self.database.citation_aliases:
                return f"Section **{headings[key]}**"
            return match.group(0)

        survey = re.sub(r"<([^<>]+)>", internal_reference, survey)
        self._test_valid_citation_threshold(survey)
        survey, references, _correct_titles, _err_titles = self.extract_and_process_citations(survey)
        return self.append_bibliography(survey, references)

    def append_bibliography(self, survey, references):
        ...
        for index, paper_id in tqdm(enumerate(references)):
            ...
            try:
                ...
                raise ValueError(f"Cannot format verified reference {paper_id}: {e}") from e
    ...
    def extract_and_process_citations(self, text: str):
        ...
        atomic_write_json(Path(self.config.BasicInfo.base_dir) / "state/xcientist/citation_errors.json",
                          {"unresolved_titles": sorted(set(err_titles))})
        if err_titles:
            raise ValueError(f"{len(set(err_titles))} citation titles remain unresolved; inspect citation_errors.json")
