...
from .paper_graph_retriever import PaperGraphRetriever
from ..utils.rich_logger import get_logger
import math
import requests
import xml.etree.ElementTree as ET
import copy
from ..utils.cluster_tables import generate_tables
...
class WorkAnalyzer:
    ...
    def generate_mla(self, paper_id: str):
        local = getattr(self.work_collector.data_manager, "xlab_paper_by_id", {}).get(paper_id)
        if local is None and self.reference_graph is not None:
            local = self.reference_graph.nodes.get(paper_id)
        if local and local.get("title"):
            authors = local.get("authors") or []
            title = local["title"]
            venue = local.get("venue") or ""
            year = local.get("year") or "n.d."
        else:
            paper = None
            ...
            year = paper.get("year", "")
        authors = [str(author.get("name") or "") if isinstance(author, dict) else str(author) for author in authors]
    ...
    def get_paper_keynote(self, paper_id: str, ds_keynote_fallback: bool = False):
        ...
        verified = getattr(self.work_collector.data_manager, "xlab_paper_by_id", {}).get(paper_id, {})
        if verified.get("verified_reference_text"):
            return verified["verified_reference_text"]
    ...
    def generate_cluster_tables(self, clusters: List[Dict]):
        self.relation_analysis_table = generate_tables(self, clusters)
        return self.relation_analysis_table
