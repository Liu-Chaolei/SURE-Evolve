...
function finishEvent(ctx) {
  return eventValue(ctx);
}
function runRelativePath(ctx, path) {
...
const READ_ONLY_INSPECTION_TOOLS = /* @__PURE__ */ new Set([
  "jq",
  "ls",
  "find",
  "rg",
...
function inspectionOperands(args) {
  const operands = [];
...
function allInspectionPathsScoped(values, ctx) {
  return values.length > 0 && values.every((value) => isScopedInspectionPath(value, ctx));
}
function isRunOrPackageInspection(command, ctx) {
  if (hasShellComposition(command) || /\b(?:curl|wget|python\s+-c|node\s+-e|perl\s+-e|nc|ssh|scp|rsync)\b|https?:\/\//i.test(
    command
  )) {
    return false;
  }
  ...
  if ((executable === "python" || executable === "python3") && args[0] === "-m" && args[1] === "json.tool") {
  ...
  if (executable === "find") {
    return !args.some((value) => FIND_MUTATING_PREDICATES.has(value)) && allInspectionPathsScoped(findRoots(args), ctx);
  }
...
function legalIncomplete(ctx, event) {
  ...
  if (exitCode !== 2 || !existsSync(paths(ctx).report) || !existsSync(paths(ctx).manifest)) {
    return false;
  }
  try {
...
function countersFromReport(report) {
...
function cliPhase(command, ctx) {
  const parsed = parsePackagedCli(command, ctx);
  if (parsed) {
    return parsed.phase;
  }
  const tokens = parseShellTokens(command);
  const scriptIndex = tokens?.[0] && /^(?:python|python3)$/.test(basename(tokens[0])) ? 1 : 0;
...
function parsePackagedCli(command, ctx) {
  ...
  const allowedPhase = phase === "init" || phase === "synthesize" || phase === "resume" || phase === "audit" ? phase : void 0;
  if (!allowedPhase) {
    return void 0;
  }
  const candidateScripts = isAbsolute(scriptToken) ? [resolve(scriptToken)] : [
    resolve(ctx.cwd || process.cwd(), scriptToken),
    resolve(ctx.packageDir || process.cwd(), scriptToken)
  ];
  if (!candidateScripts.some((candidate) => resolve(candidate) === resolve(paths(ctx).script))) {
    return void 0;
  }
  ...
  if (runDir) {
    const resolvedRunDir = isAbsolute(runDir) ? resolve(runDir) : resolve(ctx.cwd || process.cwd(), runDir);
...
function isAllowedPackagedCli(command, ctx) {
  return parsePackagedCli(command, ctx) !== void 0;
}
function runPython(ctx, args, timeout) {
...
function preStart(ctx) {
  ...
  const warnings = Array.isArray(initialized.warnings) ? initialized.warnings.filter(
    (value) => typeof value === "string"
  ) : [];
  const api = isRecord(initialized.api) ? initialized.api : {};
  const contextWindow = numeric(api.llm_context_window);
  const contextWindowSummary = contextWindow ? `; context ${Math.round(contextWindow / 1e3)}k` : "";
  const apiDiagnostic = {
    severity: "info",
  ...
  return {
    ...
    state_patch: {
      ...
      checkpoint: {
        ...
        resumable: true,
        resume_hint: "Rerun run_survey_phase.py resume with the same run directory."
      },
      next_actions: [
...
function preToolCall(ctx) {
  ...
  if (/\b(?:curl|wget)\b|api\.openai\.com|api\.semanticscholar\.org/i.test(
    command
  )) {
    return failure(
  ...
  if (/--xcientist-root|Xcientist(?:-\d+)?|XAgora|XForge|PaperGraph/i.test(
    command
  )) {
    return failure(
  ...
  if (command.includes("run_survey_phase.py") && !isAllowedPackagedCli(command, ctx)) {
    return failure(
  ...
  if (/\b(?:python(?:3)?\s+-c|node\s+-e|tee|cp|mv|touch)\b|>/.test(command) && /(?:artifacts\/(?:survey\.md|survey\.json|citations\.json|survey_report\.json)|manifest\.json)/.test(
    command
  )) {
    return failure(
...
function postToolResult(ctx) {
  ...
  const report = existsSync(paths(ctx).report) ? readJson(paths(ctx).report) : void 0;
  const counters = report ? countersFromReport(report) : void 0;
    ...
    if (legalIncomplete(ctx, event)) {
      ...
      return {
        ...
        state_patch: {
          ...
          message: "The packaged survey runtime completed and wrote an incomplete manifest; this is a recoverable quality-gate outcome, not a crash.",
    ...
    return {
      ...
      state_patch: {
        ...
        diagnostics: [
          {
            severity: "warning",
            message: timedOut ? `The literature survey ${phase} phase exceeded the tool timeout.` : `The literature survey ${phase} phase returned an error before writing a valid incomplete manifest.`,
  ...
  return {
    ...
    state_patch: {
      phase: {
        id: phase,
        label: phase === "synthesize" || phase === "smoke" ? "Literature survey artifacts generated" : phase === "audit" ? "Literature survey audit complete" : "Literature survey phase complete",
...
function preFinish(ctx) {
  ...
  const diagnostics = [
    ...
    ...blockers.map((message) => ({
      severity: "error",
      message,
      repair: "Rerun run_survey_phase.py resume after repairing the source artifacts."
    }))
  ];
  ...
  return {
    ...
    state_patch: {
      phase: {
        id: "validate",
        label: passed ? "Literature survey validated" : "Literature survey incomplete",
...
function postFinish(ctx) {
  return {
    ...
    state_patch: {
      ...
      message: ctx.run.summary ?? "Literature survey artifacts are ready for downstream XLab skills.",
      next_actions: [
...
function onResume(ctx) {
  ...
  try {
    checkpoint = existsSync(paths(ctx).checkpoint) ? readJson(paths(ctx).checkpoint) : void 0;
  } catch (error) {
    readDiagnostics.push({
      severity: "error",
      message: `Cannot read survey checkpoint: ${error instanceof Error ? error.message : String(error)}`,
      repair: "Repair or remove state/checkpoint.json, then rerun run_survey_phase.py resume."
    });
  }
  try {
    report = existsSync(paths(ctx).report) ? readJson(paths(ctx).report) : void 0;
  } catch (error) {
    readDiagnostics.push({
      severity: "error",
      message: `Cannot read survey report: ${error instanceof Error ? error.message : String(error)}`,
      repair: "Rerun run_survey_phase.py audit to regenerate artifacts/survey_report.json."
    });
  }
  const completedStages = Array.isArray(checkpoint?.completed_stages) ? checkpoint.completed_stages.filter(
    (value) => typeof value === "string"
  ) : [];
  const lastError = typeof checkpoint?.last_error === "string" ? checkpoint.last_error : void 0;
  const counters = report ? countersFromReport(report) : isRecord(checkpoint?.counts) ? Object.fromEntries(
    Object.entries(checkpoint.counts).map(([key, value]) => [key, numeric(value) ?? 0]).filter(([, value]) => value > 0)
  ) : void 0;
  const passed = report?.passed === true;
  ...
  return {
    ...
    state_patch: {
      phase: {
        id: "resume",
        label: passed ? "Literature survey ready to finish" : blockers.length > 0 || lastError ? "Literature survey blocked at quality gate" : "Resuming literature survey",
        status: passed ? "success" : blockers.length > 0 || lastError ? "incomplete" : "running"
      },
      message: completedStages.length > 0 ? `Completed stages: ${completedStages.join(", ")}.` : "No completed survey stages were found; rerun run_survey_phase.py synthesize or resume.",
      counters,
      diagnostics: [
        ...readDiagnostics,
        ...
        ...lastError ? [
          {
            severity: "error",
            message: lastError,
            repair: "Repair the failing SurveyAgent/provider/input condition and rerun run_survey_phase.py resume."
          }
        ] : []
      ],
      checkpoint: {
        id: "literature-survey-resume",
        label: completedStages.length > 0 ? `Last stage: ${completedStages.at(-1)}` : "No reusable stage recorded",
        resumable: true,
        resume_hint: passed ? "Run run_survey_phase.py audit and finish with status success." : "Rerun run_survey_phase.py resume with this run directory.",
        data: checkpoint
      },
      next_actions: passed ? [
...
function onCancel(ctx) {
  ...
  ].filter(([, path]) => existsSync(path)).map(([type, path]) => ({
    type,
    name: basename(path),
  ...
  return {
    ...
    state_patch: {
      phase: {
        id: "cancel",
        label: lastStage ? `Literature survey cancelled after ${lastStage}` : "Literature survey cancelled",
        status: "incomplete"
      },
      message: "Partial survey artifacts, logs, and checkpoints were preserved for resume.",
      counters: report ? countersFromReport(report) : void 0,
      diagnostics: report ? diagnosticsFromReport(
        report,
...
function onError(ctx) {
  return {
    ...
    state_patch: {
      ...
      message: ctx.run.errorSummary ?? ctx.run.lastRepair ?? "Literature survey stopped before completion.",
      checkpoint: {
        id: "literature-survey-error",
        label: "Partial survey preserved",
        resumable: true,
        resume_hint: "Rerun run_survey_phase.py resume with this run directory."
      }
    }
  };
}
