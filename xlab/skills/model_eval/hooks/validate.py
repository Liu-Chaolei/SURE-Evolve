...
def lifecycle(context: dict[str, Any], label: str) -> dict[str, Any]:
    point = str(context.get("point") or "pre_finish")
    workflow = read_workflow(context)
    chain = declared_chain(context)
    if point == "pre_start":
        return {"ok": True, "state_patch": {"phase": {"id": "workflow", "label": f"{label} ready", "status": "running", "progress": 0}, "message": f"Workflow stages: {' → '.join(chain)}.", "checkpoint": {"id": "workflow", "label": f"{label} lifecycle", "resumable": True, "data": {"stages": chain}}, "next_actions": ["Use xlab_stage at every declared stage boundary.", "Finish only after the evaluation report contains finite metrics and no failed cases."]}}
    if point == "pre_stage":
    ...
    if point == "on_resume":
        return {"ok": True, "state_patch": {"phase": {"id": "resume", "label": f"{label} resume point", "status": "running"}, "counters": count_statuses(workflow), "checkpoint": {"id": "workflow", "label": "Workflow resume state", "resumable": True, "data": {"stages": workflow_stages(workflow)}}, "next_actions": ["Resume the next pending or blocked evaluation stage."]}}
    if point == "on_cancel":
        return {"ok": True, "state_patch": {"phase": {"id": "cancel", "label": f"{label} cancelled", "status": "skipped"}, "checkpoint": {"id": "cancel", "resumable": False}}}
    if point == "on_error":
        event = event_value(context)
        return {"ok": True, "state_patch": {"phase": {"id": "error", "label": f"{label} error", "status": "failed"}, "diagnostics": [{"severity": "error", "code": "workflow_error", "message": str(event.get("message") or event.get("error") or f"{label} error."), "data": event}], "next_actions": ["Repair the failing stage, then use /xlab resume-run <run-id>."]}}
    return {"ok": True}


def finite_metrics(metrics: Any) -> bool:
    return is_record(metrics) and bool(metrics) and all(
...
def blocked(message: str, repair: str) -> dict[str, Any]:
    return {"ok": False, "message": message, "repair": repair, "state_patch": {"phase": {"id": "validate", "label": "Model evaluation gate", "status": "blocked"}, "diagnostics": [{"severity": "error", "code": "evaluation_gate", "message": message, "repair": repair}], "next_actions": [repair]}}


def validate_finish(context: dict[str, Any]) -> dict[str, Any]:
    report, path, error = load_artifact_json(context, "evaluation_report")
    if error:
        return blocked(error, "Create a valid evaluation_report artifact and rerun eval_run before finishing.")
    assert report is not None
    metrics = report.get("metrics")
    case_count = report.get("case_count", 0)
    if not isinstance(case_count, int) or isinstance(case_count, bool):
        return blocked("Evaluation report case_count must be an integer.", "Repair the evaluation report and rerun eval_run.")
    if case_count < 1 or report.get("failures") or not finite_metrics(metrics):
        return blocked("Evaluation report has no complete finite metrics or contains failed cases.", "Repair the runner or benchmark coverage and rerun eval_run.")
    assert is_record(metrics)
    return {"ok": True, "state_patch": {"phase": {"id": "validate", "label": "Model evaluation validated", "status": "success", "progress": 1}, "counters": {"cases": case_count, "metrics": len(metrics)}, "artifacts": [{"type": "evaluation_report", "name": "evaluation_report", "path": str(path), "status": "ready"}]}}


def dispatch(context: dict[str, Any]) -> dict[str, Any]:
    if str(context.get("point") or "pre_finish") != "pre_finish":
        return lifecycle(context, "Model evaluation")
...
def main() -> None:
    try:
        ...
        emit({"ok": False, "message": f"Model evaluation hook failed: {error}", "repair": "Fix hooks/validate.py and rerun the hook."})
