...
def lifecycle(context: dict[str, Any]) -> dict[str, Any]:
    ...
    if point == "pre_start":
        return {"ok": True, "state_patch": {"phase": {"id": "workflow", "label": "Paper reproduction workflow ready", "status": "running", "progress": 0}, "message": f"Workflow stages: {' → '.join(chain)}.", "checkpoint": {"id": "workflow", "label": "Paper reproduction lifecycle", "resumable": True, "data": {"stages": chain}}, "next_actions": ["Parse the paper, build the dataset, execute the experiment, evaluate outputs, then render the reproduction report.", "Use xlab_stage at every declared stage boundary."]}}
    if point == "pre_stage":
    ...
    if point == "on_resume":
        return {"ok": True, "state_patch": {"phase": {"id": "resume", "label": "Paper reproduction resume point", "status": "running"}, "counters": count_statuses(workflow), "checkpoint": {"id": "workflow", "label": "Workflow resume state", "resumable": True, "data": {"stages": workflow_stages(workflow)}}, "next_actions": ["Resume the next pending or blocked reproduction stage."]}}
    if point == "on_cancel":
        return {"ok": True, "state_patch": {"phase": {"id": "cancel", "label": "Paper reproduction workflow cancelled", "status": "skipped"}, "checkpoint": {"id": "cancel", "resumable": False}}}
    if point == "on_error":
        event = event_value(context)
        return {"ok": True, "state_patch": {"phase": {"id": "error", "label": "Paper reproduction workflow error", "status": "failed"}, "diagnostics": [{"severity": "error", "code": "workflow_error", "message": str(event.get("message") or event.get("error") or "Paper reproduction workflow error."), "data": event}], "next_actions": ["Repair the failing stage, then use /xlab resume-run <run-id>."]}}
    return {"ok": True}


def blocked(message: str, repair: str) -> dict[str, Any]:
    return {"ok": False, "message": message, "repair": repair, "state_patch": {"phase": {"id": "validate", "label": "Reproduction report gate", "status": "blocked"}, "diagnostics": [{"severity": "error", "code": "reproduction_gate", "message": message, "repair": repair}], "next_actions": [repair]}}
...
def validate_finish(context: dict[str, Any]) -> dict[str, Any]:
    ...
    report, path, error = load_artifact_json(context, "reproduction_report")
    if error:
        return blocked(error, "Create a valid reproduction_report artifact before finishing.")
    assert report is not None
    comparison = report.get("comparison")
    stage_artifacts = report.get("stage_artifacts")
    invalid_success = finish.get("status") == "success" and (
        report.get("status") != "success"
        or not is_record(comparison)
        or not comparison
        or not isinstance(stage_artifacts, list)
        or len(stage_artifacts) < 4
    )
    if invalid_success:
        return blocked("Reproduction success requires all stage artifacts and aligned metrics within tolerance.", "Repair missing stages or finish as incomplete with explicit limitations.")
    return {"ok": True, "state_patch": {"phase": {"id": "validate", "label": "Reproduction report validated", "status": "success", "progress": 1}, "counters": {"aligned_metrics": len(comparison) if is_record(comparison) else 0}, "artifacts": [{"type": "reproduction_report", "name": "reproduction_report", "path": str(path), "status": "ready"}]}}
...
def main() -> None:
    try:
        ...
        emit({"ok": False, "message": f"Paper reproduction hook failed: {error}", "repair": "Fix hooks/validate.py and rerun the hook."})
