...
function providerStats(path: string): {
  logical: number;
  successful: number;
  failed: number;
} {
  ...
  for (const line of readFileSync(path, "utf-8").split(/\r?\n/)) {
    ...
    try {
      ...
      if (
        value.operation === "semantic_scholar.get_paper_citations" ||
...
function eventManifestPath(ctx: XlabHookContext): string | undefined {
...
function cliPhase(command: string): "collect" | "download" | "audit" | "other" {
  const match = command.match(
    /paper_collect\.py\s+(collect|resume|download|audit|run|smoke)\b/,
  );
  if (
    match?.[1] === "collect" ||
    match?.[1] === "resume" ||
...
interface PackagedCliCommand extends ParsedCommand {
  phase: "collect" | "resume" | "download" | "audit";
}
...
function resolveRunArgument(commandPath: string, ctx: XlabHookContext): string {
  return isAbsolute(commandPath)
    ? resolve(commandPath)
    : resolve(ctx.cwd, commandPath);
}

function isPackagedScriptPath(commandPath: string, ctx: XlabHookContext): boolean {
...
function isAllowedPythonExecutable(value: string, ctx: XlabHookContext): boolean {
...
function packagedScriptArgument(
  parsed: ParsedCommand,
  ctx: XlabHookContext,
): { script: string; args: string[] } | undefined {
...
function runDirValuesAreExpected(values: string[], ctx: XlabHookContext): boolean {
  return (
    values.length === 1 && resolveRunArgument(values[0] ?? "", ctx) === resolve(ctx.runDir)
  );
}
...
function parsePackagedCliCommand(
  ...
  return { ...parsed, phase: phase as PackagedCliCommand["phase"] };
}
...
function packagedCliRejection(
  command: string,
  ctx: XlabHookContext,
): { repair: string; message: string } | undefined {
  const parsed = parseSimpleCommand(command);
  if (!parsed) {
    return {
      repair:
        "Run paper_collect.py as one simple command without shell control operators or redirects.",
  ...
  if (!script || !isPackagedScriptPath(script.script, ctx)) {
    return {
      ...
      message:
        "Blocked paper_collect.py invocation outside the active skill package.",
    };
  }
  const phase = script.args[0];
  if (!ALLOWED_CLI_PHASES.has(phase)) {
    return {
      repair:
        "Use only paper_collect.py collect, resume, download, or audit in the slash-command runtime. The run and smoke commands are not workflow phases here.",
  ...
  if (!runDirValuesAreExpected(optionValues(script.args.slice(1), "--run-dir"), ctx)) {
    return {
      ...
      message:
        "Blocked paper_collect CLI invocation for a different run directory.",
    };
  }
  return undefined;
}
...
function areInspectionPathsScoped(
  parsed: ParsedCommand,
  ctx: XlabHookContext,
): boolean {
...
export function preStart(ctx: XlabHookContext): XlabHookResult {
  ...
  if (missing.length > 0) {
    return failure(
      "Restore the missing files inside the paper_collect skill package.",
      `Paper collection package is incomplete: ${missing.join(", ")}`,
    );
  }
  ...
  if (initialization.error || initialization.status !== 0) {
    ...
    return failure(
      'Use /xlab collect-papers "<topic>" --target-papers N --max-papers N; repeat --facet "<facet>" as needed.',
      `Invalid paper collection arguments: ${detail}`,
    );
  }
  ...
  try {
    ...
    return failure(
      "Run paper_collect.py init manually and inspect its JSON output.",
      `Paper collection initialization returned invalid JSON: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  return {
    ok: true,
    state_patch: {
      ...
      checkpoint: {
        ...
        resumable: true,
        resume_hint:
          "Rerun the interrupted collect, download, or audit phase; completed work is reused.",
      },
    },
  };
}
...
export function preToolCall(ctx: XlabHookContext): XlabHookResult {
  ...
  if (toolName.startsWith("mcp__")) {
    return failure(
      "Run paper_collect.py run or resume. Provider access is implemented by the packaged Python CLI.",
      "MCP search is invoked by the packaged Python client, not as an injected tool call.",
    );
  }
  if (toolName !== "bash") {
    if (toolName === "read") {
      ...
      return failure(
        "Read only files inside the active run directory or paper_collect package.",
        "Blocked read outside the paper_collect run/package scope.",
      );
    }
    return failure(
      "Use only read-only inspection plus the packaged paper_collect.py CLI; final artifacts must be produced by the CLI.",
      `paper_collect does not allow the ${toolName || "unknown"} tool during a run.`,
    );
  }
  ...
  if (
    ...
    return failure(
      "Use the packaged paper_collect.py CLI so retries, checkpoints, normalization, and audit evidence remain consistent.",
      "Direct provider HTTP commands bypass the collection contract.",
    );
  }
  if (
    /\bmcp(?:__|[-_. ]?server|[-_. ]?config|\s+list|\s+inspect)/i.test(command)
  ) {
    return failure(
      "Use the configured WEB_SEARCH_MCP_URL through paper_collect.py; do not bypass the packaged provider client.",
      "MCP search configuration is consumed by the paper_collect provider client.",
    );
  }
  ...
  if (
    ...
    /(?:artifacts\/(?:papers\.manifest\.json|collection_report\.json|metadata\/edges\.jsonl)|manifest\.json)/.test(
      command,
    )
  ) {
    return failure(
      "Produce final collection artifacts through paper_collect.py so provider evidence, audit counters, and manifest paths remain canonical.",
      "Blocked an ad hoc command that could overwrite paper_collect final artifacts.",
    );
  }
  if (
    ...
    return failure(
      "Run the packaged paper_collect phase CLI, or use read-only inspection scoped to the run/package directory.",
      "Blocked a Bash command outside the paper_collect control plane.",
    );
  }
  return { ok: true };
}
...
export function preFinish(ctx: XlabHookContext): XlabHookResult {
  ...
  if (!existsSync(packagePaths.collection)) {
    return failure(
      "Run paper_collect.py collect before download and audit.",
      "Missing artifacts/papers.manifest.json.",
    );
  }
  const python = ctx.run.environment?.executable ?? "python";
  ...
  if (result.error || result.status !== 0) {
    return failure(
      "Run the Python audit manually, repair the reported artifact, and retry xlab_finish.",
      `Paper collection audit failed: ${result.error?.message ?? result.stderr.trim() ?? `exit ${result.status}`}`,
    );
  }
  ...
  if (!submittedManifestPath || resolve(submittedManifestPath) !== resolve(packagePaths.manifest)) {
    return failure(
      "Call xlab_finish with the canonical manifest generated by paper_collect.py audit.",
      `paper_collect requires manifest_path ${packagePaths.manifest}.`,
    );
  }
  ...
  try {
    ...
    return failure(
      "Repair the collection JSON artifacts and rerun paper_collect.py audit.",
      `Cannot read collection audit artifacts: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  const manifestArtifacts = Array.isArray(manifest.artifacts)
    ? manifest.artifacts.filter((entry): entry is ManifestArtifact =>
        isRecord(entry),
      )
    : [];
  for (const [type, path, canonical] of [
    ...
    if (!isWithin(path, ctx.cwd) && !isWithin(path, ctx.runDir)) {
      return failure(
        "Use the run-local artifact paths written by paper_collect.py audit.",
        `Artifact ${type} points outside the project/run workspace: ${path}`,
      );
    }
    if (resolve(path) !== resolve(canonical)) {
      return failure(
        "Use the canonical manifest generated by paper_collect.py audit before calling xlab_finish.",
  ...
  if (finishStatus === "success" && !passed) {
    ...
    return failure(
      ...
      blockers.join(" ") || "Paper collection did not pass the success gates.",
      counters,
    );
  }
  const reportStatus = passed ? "success" : "incomplete";
  const generatedStatus =
    typeof manifest.status === "string" ? manifest.status : reportStatus;
  if (generatedStatus !== reportStatus) {
    return failure(
      "Rerun paper_collect.py audit so manifest status matches collection_report.json.",
  ...
  const collectedCount =
    numeric(collection.collected_count) ?? counters.collected_papers;
  return {
    ok: true,
    state_patch: {
      phase: {
...
export function onResume(ctx: XlabHookContext): XlabHookResult {
  ...
  try {
    ...
    readDiagnostics.push({
      ...
      repair:
        "Rerun paper_collect.py audit to regenerate artifacts/collection_report.json.",
    });
  }
  const counts = isRecord(report?.counts) ? report.counts : {};
  const counters = {
    api_calls:
      numeric(counts.logical_api_calls) ??
      ...
      diagnostics: [
        ...
        ...(report
          ? diagnosticsFromReport(
              report,
              "Repair the reported blocker, then rerun paper_collect.py resume, download, or audit.",
            )
          : []),
      ],
      checkpoint: {
...
export function onCancel(): XlabHookResult {
  return {
    ...
    state_patch: {
      ...
      message:
        "Provider responses, partial metadata, and PDF part files were preserved for resume.",
    },
  };
}
