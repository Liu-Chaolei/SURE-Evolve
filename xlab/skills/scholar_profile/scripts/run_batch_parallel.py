...
def run_one_scholar(
    ...
    if auto_discover_sources and not _sources_file_is_valid(sources_file):
        try:
            ...
            search_endpoint = (env.get("WEB_SEARCH_MCP_URL") or "").strip()
            print(f"[INFO] Auto-discovering sources for {row.author}...")
            ok, msg = collect_and_save_sources(
                row.author, row.dblp_url, str(sources_file), search_endpoint
            )
            if ok:
...
def main() -> int:
    ...
    search_endpoint = (os.getenv("WEB_SEARCH_MCP_URL") or "").strip()
    if search_endpoint:
        env["WEB_SEARCH_MCP_URL"] = search_endpoint
    ...
    print(f"[INFO] WEB_SEARCH_MCP_URL: {search_endpoint or 'not set'}")
