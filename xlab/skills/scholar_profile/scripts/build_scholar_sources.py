...
Data sources:
  ...
  - HTTP MCP web search: interviews, lab homepages, news
"""
...
def web_search(query: str, endpoint: str, max_results: int = 5) -> List[Tuple[str, str]]:
    """Search the configured stateless MCP endpoint, returning [(title, url), ...]."""
    if not endpoint:
        return []
    try:
        ...
        initialize = {
            ...
            "params": {
                ...
                "capabilities": {},
                "clientInfo": {"name": "xlab-scholar-profile", "version": "1.0.0"},
            },
        }
        init_response = requests.post(endpoint, json=initialize, timeout=120)
        init_response.raise_for_status()
        init_value = init_response.json()
        if (
            not isinstance(init_value, dict)
            or init_value.get("jsonrpc") != "2.0"
            or init_value.get("id") != initialize_id
            or not isinstance(init_value.get("result"), dict)
        ):
            raise RuntimeError("invalid MCP initialize response")
        initialized = {"jsonrpc": "2.0", "method": "notifications/initialized"}
        try:
            requests.post(endpoint, json=initialized, timeout=120)
        except Exception:
            logger.debug("MCP initialized notification failed", exc_info=True)
        payload = {
            "jsonrpc": "2.0",
            "id": str(uuid.uuid4()),
            "method": "tools/call",
            "params": {
                "name": "search",
                "arguments": {"query": query, "max_sources": min(50, max(1, max_results))},
            },
        }
            ...
            for item in result.get("content", []) if isinstance(result, dict) else []:
                if isinstance(item, dict) and item.get("type") == "text":
                    ...
                    if isinstance(decoded, dict):
                        ...
                        break
        output = []
        for item in structured.get("results", []) if isinstance(structured, dict) else []:
        ...
        logger.warning(f"Web search MCP failed for '{query}': {type(exc).__name__}: {exc}")
        return []


def search_scholar_resources(author_name: str, search_endpoint: str) -> ScholarSources:
    """Search for various scholar resources using the configured MCP endpoint."""
    sources = ScholarSources(author_name=author_name)

    if not search_endpoint:
        logger.info("No WEB_SEARCH_MCP_URL provided, skipping web search")
        return sources
    ...
    for category, query in queries.items():
        results = web_search(query, search_endpoint, max_results=5)
...
def collect_all_sources(
    ...
    web_sources = search_scholar_resources(author_name, search_endpoint)
...
def collect_and_save_sources(
    ...
    try:
        sources = collect_all_sources(author_name, dblp_url, search_endpoint)
...
def main():
    ...
    parser.add_argument("--web-search-url", default="", help="HTTP MCP web-search endpoint (or set WEB_SEARCH_MCP_URL)")
    ...
    search_endpoint = (args.web_search_url or os.environ.get("WEB_SEARCH_MCP_URL") or "").strip()
    skip_existing = str(args.skip_existing).strip().lower() in {"1", "true", "yes", "y", "on"}

    # Batch mode
    if args.csv:
        _run_batch(args.csv, search_endpoint, skip_existing)
        return
    ...
    ok, msg = collect_and_save_sources(
        args.author.strip(), args.dblp_url.strip(), args.output.strip(), search_endpoint
    )
...
def run_single(scholar_name: str, output_dir: str, csv_path: str = None,
               seed_urls: list = None, search_endpoint: str = "") -> str:
    ...
    Args:
        ...
        search_endpoint: HTTP MCP web-search endpoint.
    ...
    if not search_endpoint:
        search_endpoint = os.environ.get("WEB_SEARCH_MCP_URL", "")

    ok, msg = collect_and_save_sources(
        scholar_name, dblp_url, str(sources_path), search_endpoint
    )
    if not ok:
