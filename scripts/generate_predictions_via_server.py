...
from sure_eval.protocols import ProtocolResolver, canonical_sha256
from sure_eval.results import AppendOnlyResultStore, file_sha256
...
def _dataset_identity(
    ...
    parts = canonical_dataset.rsplit("__", 1)
    if len(parts) != 2 or not all(parts) or "__" in parts[0]:
        raise ValueError(
            "dataset identity must be '<source_dataset_name>__<version_id>' without a task suffix"
        )
    split = "test"
    if samples and isinstance(samples[0].get("metadata"), dict):
        split = str(samples[0]["metadata"].get("split") or split)
    return {
        "id": canonical_dataset,
        "name": parts[0],
        "version": parts[1],
        "split": split,
        "manifest_sha256": file_sha256(jsonl_path),
...
def _write_json_once_or_verify(path: Path, payload: dict[str, Any]) -> None:
    if path.exists():
        ...
        if existing != payload:
            raise ValueError(f"immutable artifact differs from the resumed run: {path}")
        return
...
def _write_prediction_manifest(
    *,
    ...
    files = [
        {
            "role": "compatibility_predictions",
            "path": str(prediction_path.relative_to(model_root)),
            "sha256": file_sha256(prediction_path),
        },
        {
            "role": "structured_predictions",
            "path": str(structured_prediction_path.relative_to(model_root)),
            "sha256": file_sha256(structured_prediction_path),
        },
    ]
    if output_audio_dir.is_dir():
        for audio_path in sorted(path for path in output_audio_dir.rglob("*") if path.is_file()):
            files.append(
                {
                    "role": f"generated_audio:{audio_path.relative_to(output_audio_dir)}",
                    "path": str(audio_path.relative_to(model_root)),
                    "sha256": file_sha256(audio_path),
                }
            )
    sample_ids = [str(sample.get("key", "")) for sample in samples]
    payload = {
    ...
    path = manifest_dir / f"{dataset['id']}.prediction_manifest.json"
    _write_json_once_or_verify(path, payload)
    return path


def main() -> int:
    parser = argparse.ArgumentParser(description="Generate predictions by calling a model-local MCP server")
    parser.add_argument("--model-id", required=True, help="Canonical model id in the published NFS registry")
    parser.add_argument("--dataset", required=True, help="Canonical dataset name")
    parser.add_argument("--inference-id", required=True, help="Immutable local inference run id")
    ...
    model_info = ModelRegistry(storage=storage).require_verified(args.model_id)
    model_dir = model_info.path
    resolved_protocol = ProtocolResolver().resolve(args.protocol_id, model_info)
    protocol_identity = {
        "id": resolved_protocol.protocol_id,
        "version": resolved_protocol.protocol_version,
        "effective_params_sha256": resolved_protocol.effective_params_sha256,
    }
    store = AppendOnlyResultStore(
        model_info.name,
        model_info.artifact_sha256,
        storage=storage,
    )
    if args.resume:
        ...
        if not run_dir.is_dir():
            raise FileNotFoundError(f"cannot resume missing inference run: {run_dir}")
    else:
        run_dir = store.reserve_inference_run(args.inference_id)
    predictions_dir = run_dir / "predictions"
    logs_dir = predictions_dir / "logs"
    predictions_dir.mkdir(parents=True, exist_ok=True)
    logs_dir.mkdir(parents=True, exist_ok=True)
    _write_json_once_or_verify(run_dir / "protocol_resolution.json", resolved_protocol.to_dict())
    ...
    dataset_identity = _dataset_identity(canonical_dataset, jsonl_path, samples)
    ...
    env["SURE_EVAL_PROTOCOL_ID"] = resolved_protocol.protocol_id
    env["SURE_EVAL_PROTOCOL_EFFECTIVE_SHA256"] = resolved_protocol.effective_params_sha256
    env["PYTHONDONTWRITEBYTECODE"] = "1"
    for key, value in resolved_protocol.model_params.items():
    ...
    if args.resume and not pending_keys:
        ...
        current_dataset_status["num_pending_samples"] = 0
        status_path.write_text(json.dumps(status_payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
        logger.info(
        ...
        prediction_manifest_path = _write_prediction_manifest(
            model_root=store.model_root,
            run_dir=run_dir,
            model_id=model_info.name,
            model_artifact_sha256=model_info.artifact_sha256,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix=f"sure-eval-{canonical_dataset}-audio-") as scratch:
                ...
                for sample in samples:
                    ...
                    current_dataset_status["num_generated_samples"] = len(samples) - len(current_pending_keys)
                    current_dataset_status["num_pending_samples"] = len(current_pending_keys)
                    status_path.write_text(
                        json.dumps(status_payload, indent=2, ensure_ascii=False) + "\n",
                        encoding="utf-8",
                    )
                    if (
                        current_dataset_status["num_generated_samples"] % PREDICTION_SNAPSHOT_INTERVAL == 0
