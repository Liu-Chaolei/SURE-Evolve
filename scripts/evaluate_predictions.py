#!/usr/bin/env python3
"""
Legacy compatibility evaluator for v1 artifacts.

This is not a v2 main-flow entrypoint. New runs use run_evaluation_bridge.py.
The compatibility implementation is deterministic by design:
...
_ASR_ROUTE_COVERAGE_KEYS = {
    "num_ref_utts",
    "num_hyp_utts",
    "num_hyp_missing_utts",
    "num_hyp_extra_utts",
}
...
def _clean_source_info(source_info: dict[str, Any]) -> dict[str, Any]:
    cleaned: dict[str, Any] = {}
    for key, value in source_info.items():
        if value is None or value == "":
            continue
        if key.endswith("_root") or key.endswith("_path"):
            cleaned[key] = _display_path(value)
        else:
            cleaned[key] = value
    return cleaned


def _dataset_source_info(
    dataset_manager: Any,
    dataset_name: str,
    jsonl_path: Path,
    samples: list[dict[str, Any]],
) -> dict[str, Any]:
    getter = getattr(dataset_manager, "get_source_info", None)
    if callable(getter):
        try:
            source_info = getter(dataset_name)
        except Exception as exc:
            logger.warning("Failed to read dataset source info", dataset=dataset_name, error=str(exc))
        else:
            if isinstance(source_info, dict) and source_info:
                return _clean_source_info(source_info)

    metadata = samples[0].get("metadata") if samples and isinstance(samples[0].get("metadata"), dict) else {}
    if metadata.get("source") == "aispeech_ds_pool":
        return _clean_source_info(
            {
...
def _public_result_details(task: str, details: dict[str, Any]) -> dict[str, Any]:
    if task == "ASR":
        return {key: value for key, value in details.items() if key not in _ASR_ROUTE_COVERAGE_KEYS}
    return details
...
def evaluate_prediction_file(
    ...
    dataset_source = _dataset_source_info(dataset_manager, canonical_name, jsonl_path, samples)
    ...
    try:
        ...
        if task == "TTS":
            ...
            raise ValueError(
                f"legacy compatibility evaluator has no route-backed implementation for task {task!r}; "
                "use scripts/run_evaluation_bridge.py"
            )
    finally:
    ...
    public_details = _public_result_details(task, details)
...
def _write_protocol_yaml(
    ...
    *,
    ...
    for result in results:
        source_info = result.get("dataset_source") or {}
        ...
        if source_info:
            dataset["source"] = _clean_source_info(source_info)
        metric = str(result["metric"])
...
def _report_jsonl_line(
    *,
    ...
    source_info = _clean_source_info(result.get("dataset_source") or {})
    dataset_payload = {
        "name": result["dataset"],
        "task": result["task"],
        "language": result["language"],
        ...
        dataset_payload.update(
            {
                ...
                "dataset_id": source_info.get("dataset_id") or result["dataset"],
                ...
                "naming_policy": source_info.get("naming_policy"),
            }
        )
    return {
...
def _payload_result_row(result: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "dataset_source": _clean_source_info(result.get("dataset_source") or {}),
        "task": result["task"],
        "language": result["language"],
...
def _legacy_result_from_payload_row(result: dict[str, Any]) -> dict[str, Any]:
    ...
    internal = {
        ...
        "dataset_source": result.get("dataset_source") or {},
...
def main() -> int:
    ...
    parser.add_argument("--protocol-id", type=str, default="standard_system", help="Inference protocol ID")
