...
Arguments:
  ...
  --run-dir: Specify the run directory (optional, defaults to /hpc_stor03/sjtu_home/chaolei.liu/data/experiments/SURE-Evolve/runs/{agent}_{timestamp}/)
"""
...
DEFAULT_RUN_ROOT = Path(
    "/hpc_stor03/sjtu_home/chaolei.liu/data/experiments/SURE-Evolve/runs"
)
...
Examples:
  ...
  # Specify an exact run directory
  python run.py --agent minimal --task "analyze data" --run-dir runs/my_experiment

  # Specify a timestamped run name under the default run root
  ...
  python run.py --agent minimal --task-file tasks.json --parallel
    ...
    parser.add_argument(
        "--run-dir",
        help=(
            f"Run directory. Default: {DEFAULT_RUN_ROOT}/{{agent}}_{{timestamp}}/. "
            f"A bare name such as 'math' becomes {DEFAULT_RUN_ROOT}/math_{{timestamp}}/; "
...
def resolve_run_dir(agent_name: str, run_dir_arg: str | None) -> Path:
    ...
    if run_dir_arg is None:
        ...
        return DEFAULT_RUN_ROOT / f"{run_name}_{timestamp}"
