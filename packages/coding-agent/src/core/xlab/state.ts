...
const PHASE_STATUSES = new Set<XlabDisplayPhaseStatus>([
	"pending",
	"running",
	"success",
	"failed",
...
export interface XlabStateValidationResult {
	ok: boolean;
...
function parsePhase(value: unknown): { value?: XlabDisplayPhase; message?: string } {
	if (value === undefined) {
		return {};
	}
	if (!isRecord(value)) {
	...
	if (value.status !== undefined) {
		if (typeof value.status !== "string" || !PHASE_STATUSES.has(value.status as XlabDisplayPhaseStatus)) {
			return { message: "phase.status must be a valid XLab display phase status." };
		}
		parsed.status = value.status as XlabDisplayPhaseStatus;
	}
...
function parseDiagnostics(value: unknown): { value?: XlabDisplayDiagnostic[]; message?: string } {
	if (value === undefined) {
		return {};
	}
	if (!Array.isArray(value)) {
	...
	const parsed: XlabDisplayDiagnostic[] = [];
	for (const [index, item] of value.entries()) {
		if (!isRecord(item)) {
		...
		const diagnostic: XlabDisplayDiagnostic = { message: item.message };
		if (item.severity !== undefined) {
			...
			diagnostic.severity = item.severity as XlabDisplayDiagnostic["severity"];
		}
...
function parseArtifacts(value: unknown): { value?: XlabDisplayArtifact[]; message?: string } {
	if (value === undefined) {
		return {};
	}
	if (!Array.isArray(value)) {
	...
	const parsed: XlabDisplayArtifact[] = [];
	for (const [index, item] of value.entries()) {
		if (!isRecord(item)) {
			return { message: `artifacts[${index}] must be an object.` };
		}
		const artifact: XlabDisplayArtifact = {};
		...
		if (item.status !== undefined) {
			...
			artifact.status = item.status as XlabDisplayArtifact["status"];
		}
...
function parseCheckpoint(value: unknown): { value?: XlabDisplayCheckpoint; message?: string } {
	if (value === undefined) {
		return {};
	}
	if (!isRecord(value)) {
	...
	const checkpoint: XlabDisplayCheckpoint = {};
	if (value.id !== undefined) {
		if (typeof value.id !== "string") {
...
export function normalizeXlabDisplayStatePatch(value: unknown): XlabStateValidationResult {
	if (!isRecord(value)) {
...
export function mergeXlabDisplayState(
	previous: XlabDisplayState | undefined,
	patch: XlabDisplayState,
): XlabDisplayState {
	return {
		...previous,
		...patch,
...
export function formatXlabDisplayStatus(command: string, runId: string, state?: XlabDisplayState): string {
	...
	return suffix ? `XLab ${command} ${runId}: ${suffix}` : `XLab ${command} ${runId}`;
}
