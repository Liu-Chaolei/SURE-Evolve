...
describe("SelectList", () => {
	...
	it("shows the selected description on a second line in narrow terminals", () => {
		const items = [
			{ value: "collect-papers", label: "collect-papers", description: "Collect a focused paper corpus." },
			{ value: "run-experiment", label: "run-experiment", description: "Run a reproducible experiment." },
		];
		...
		assert.ok(rendered[0].includes("collect-papers"));
		assert.ok(rendered[1].includes("Collect a focused paper corpus."));
		assert.ok(rendered[2].includes("run-experiment"));
		assert.ok(!rendered[2].includes("Run a reproducible experiment."));
	});
