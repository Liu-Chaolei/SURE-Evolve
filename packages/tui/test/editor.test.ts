...
describe("Editor component", () => {
	...
	describe("Autocomplete", () => {
		it("advances through hierarchical slash completions with Enter without submitting", async () => {
			const editor = new Editor(createTestTUI(), defaultEditorTheme);
				...
				[
					{
						...
						getArgumentCompletions: (prefix) => {
							if (prefix === "") {
								return [{ value: "collect-papers ", label: "collect-papers", description: "Collect papers." }];
							}
							if (prefix === "collect-papers ") {
								return [
									{
										value: "collect-papers --workspace ",
										label: "--workspace <slug>",
										description: "Select an XLab workspace.",
									},
								];
							}
							if (prefix === "collect-papers --workspace ") {
								return [
									{
										value: "collect-papers --workspace graph-lab ",
										label: "graph-lab",
										description: "Graph learning workspace",
									},
								];
							}
							return null;
			...
			for (const character of "/xl") editor.handleInput(character);
			await flushAutocomplete();
			assert.strictEqual(editor.isShowingAutocomplete(), true);

			editor.handleInput("\r");
			await flushAutocomplete();
			assert.strictEqual(editor.getText(), "/xlab ");
			assert.strictEqual(editor.isShowingAutocomplete(), true);
			assert.deepStrictEqual(submitted, []);

			editor.handleInput("\r");
			await flushAutocomplete();
			assert.strictEqual(editor.getText(), "/xlab collect-papers ");
			assert.strictEqual(editor.isShowingAutocomplete(), true);
			assert.deepStrictEqual(submitted, []);

			editor.handleInput("\r");
			await flushAutocomplete();
			assert.strictEqual(editor.getText(), "/xlab collect-papers --workspace ");
			assert.strictEqual(editor.isShowingAutocomplete(), true);
			assert.deepStrictEqual(submitted, []);

			editor.handleInput("\r");
			await flushAutocomplete();
			assert.strictEqual(editor.getText(), "/xlab collect-papers --workspace graph-lab ");
			assert.strictEqual(editor.isShowingAutocomplete(), false);
			assert.deepStrictEqual(submitted, []);

			editor.handleInput("\r");
			assert.deepStrictEqual(submitted, ["/xlab collect-papers --workspace graph-lab"]);
		});

		it("advances hierarchical slash completions with Tab", async () => {
			const editor = new Editor(createTestTUI(), defaultEditorTheme);
			const provider = new CombinedAutocompleteProvider(
				[
					{
