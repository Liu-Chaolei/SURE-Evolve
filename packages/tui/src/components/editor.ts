...
export class Editor implements Component, Focusable {
		...
		if (this.autocompleteState && this.autocompleteList) {
			...
			if (kb.matches(data, "tui.input.tab")) {
				...
				if (selected && this.autocompleteProvider) {
					...
					this.cancelAutocomplete();
					if (this.onChange) this.onChange(this.getText());
					if (this.shouldContinueSlashCompletion()) {
	...
	private insertCharacter(char: string, skipUndoCoalescing?: boolean): void {
		...
		if (!this.autocompleteState) {
			...
			// A space opens the next slash-command argument level.
	...
	private shouldContinueSlashCompletion(): boolean {
		const currentLine = this.state.lines[this.state.cursorLine] || "";
		const beforeCursor = currentLine.slice(0, this.state.cursorCol);
		return this.isInSlashCommandContext(beforeCursor) && /\s$/.test(beforeCursor);
	}
	...
	private async runAutocompleteRequest(
		...
		let usedForce = options.force;
		let suggestions = await this.autocompleteProvider.getSuggestions(
			this.state.lines,
			this.state.cursorLine,
			this.state.cursorCol,
		...
		if (!suggestions && options.explicitTab && !options.force) {
		...
		if (usedForce && options.explicitTab && suggestions.items.length === 1) {
		...
		this.applyAutocompleteSuggestions(suggestions, usedForce ? "force" : "regular");
		this.tui.requestRender();
	}
