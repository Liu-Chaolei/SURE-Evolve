...
export interface InputOptions {
	/** Replace each grapheme with this character while rendering. The original value is still returned on submit. */
...
export class Input implements Component, Focusable {
	...
	render(width: number): string[] {
		...
		let visibleText = "";
		const beforeCursorValue = this.value.slice(0, this.cursor);
		const displayValue = this.maskCharacter
			? this.maskCharacter.repeat([...segmenter.segment(this.value)].length)
			: this.value;
		const displayCursor = this.maskCharacter
			? this.maskCharacter.repeat([...segmenter.segment(beforeCursorValue)].length).length
			: this.cursor;
