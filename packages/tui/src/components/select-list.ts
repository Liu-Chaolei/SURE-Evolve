...
export class SelectList implements Component {
	...
	render(width: number): string[] {
		...
		for (let i = startIndex; i < endIndex; i++) {
			...
			if (isSelected && descriptionSingleLine && width <= 40) {
				const description = truncateToWidth(descriptionSingleLine, Math.max(1, width - 4), "");
				lines.push(`  ${this.theme.description(description)}`);
			}
		}
