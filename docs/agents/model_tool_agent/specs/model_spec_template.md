...
| `weights.repo_id` | string | ✅ | 权重仓库标识；用于推导模型目录和 Docker 镜像名 |
...
```text
model_dir_slug = weights.repo_id.replace("/", "__")
image_slug = model_dir_slug.lower()
```

`model_name` 只用于显示和报告，不能参与 `src/sure_eval/models/{model_dir_slug}/`
目录命名或 `docker.v2.aispeech.com/sjtu/sjtu_yukai-dujunhao-sure_{image_slug}:vX.Y`
镜像命名。
