...
Docker validation scripts must not treat the existence or non-empty size of
`sample_output.json` as sufficient proof of success. Some wrappers write
`sample_output.json` for failed runs to preserve diagnostics. The script must
also assert an explicit pass signal, for example:

- `sample_output.json` contains `"overall": "PASSED"` for wrappers with an
  overall verdict field; or
- `validation.log` contains the expected `VALIDATE_CONTRACT` passed record; or
- a task-specific validation report contains its formal pass field.

Only after this semantic pass check may the script append
