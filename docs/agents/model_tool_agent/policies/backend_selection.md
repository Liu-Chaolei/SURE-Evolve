...
Backend 选择只决定 Phase-1 / local validation 用什么环境跑通最小可调用路径。
它不决定 local 模型是否需要最终容器化。`deployment_type: local` 的模型即使
Phase-1 选择 `uv`、`pixi` 或 `conda`，本地验证通过后仍必须进入
`LOCAL_DEPLOYMENT_GATE`，继续 Docker build / Docker validate / Docker push /
registry pull verify / VC validate。
...
记录决策到 backend_choice.json
    ↓
如果 deployment_type == local 且 Phase-1 通过
    ↓
进入 LOCAL_DEPLOYMENT_GATE（不受 chosen_backend 是否为 docker 影响）
```
...
2. 选择 uv（更轻量、更快）
3. 记录理由："uv satisfies phase-1 local validation requirements"
4. 明确记录："Docker is still required after local validation because deployment_type is local"
...
4. 对 local 模型继续执行 Docker/VC 验证；若本地 GPU OOM，按 VC GPU 队列验证规则处理
...
2. **不得过度工程化 Phase-1** - 本地最小验证能用 uv 就不必强行先用 docker
3. **不得无记录切换** - 所有 backend 变更必须记录在案
4. **不得忽略基础设施约束** - 网络、存储、权限必须检查
5. **不得混淆 backend 与最终部署** - local 模型不能因为 Phase-1 chosen_backend 不是 docker 而跳过 Docker/Registry/VC 闭环
