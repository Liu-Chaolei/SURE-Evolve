...
For `deployment_type: local`, environment routing has two phases:

1. Phase-1 / local validation: read the selected backend playbook (`env_uv`,
   `env_pixi`, `env_conda`, `env_docker`, etc.).
2. Deployment closure: after local validation passes, read `playbooks/env_docker.md`
   before `LOCAL_DEPLOYMENT_GATE`, even if the Phase-1 backend was not docker.

Do not record `env_docker.md` as permanently skipped for a local model. It may be
skipped only during Phase-1 context selection and must be loaded for the Docker /
registry / VC stages.
...
| `deployment_type: local` after Phase-1 passes | `playbooks/env_docker.md` | Mandatory before Docker build/validate/push/pull and VC validation, regardless of Phase-1 backend. |
