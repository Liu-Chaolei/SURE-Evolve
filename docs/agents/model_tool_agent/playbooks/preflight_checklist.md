...
**Check**:
```bash
ls -la src/sure_eval/models/{model_dir_slug}/
touch src/sure_eval/models/{model_dir_slug}/.write_test
rm src/sure_eval/models/{model_dir_slug}/.write_test
```

`model_dir_slug` must be derived from `model.spec.yaml` `weights.repo_id` by
replacing `/` with `__`.
