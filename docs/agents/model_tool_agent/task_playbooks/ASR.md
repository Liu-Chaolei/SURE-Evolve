...
- ASR metric 脚本索引：
  - `src/sure_eval/evaluation/tasks/asr/metrics.py`
...
- ASR metric 环境：
  ...
  - `src/sure_eval/evaluation/tasks/asr/README.md`

`CERMetric` / `WERMetric` 是正式 SURE ASR scoring 的类名封装，内部使用
`tasks/asr` route 和 `nodes/scoring/wenet_wer/wenet_compute_cer.py`，不是旧的简单
edit-distance 实现。

强制要求：

- ASR re-onboarding、model onboarding、Docker validation 产出的 WER/CER artifact
  必须调用 `src/sure_eval/evaluation/tasks/asr/metrics.py` 中的 `CERMetric` /
  `WERMetric`，不得在 `validate.py`、一次性脚本或 wrapper 中重新实现第二套
  edit-distance / normalization 逻辑。
- 允许 `validate.py` 只负责保存已有 reference / prediction；metric 可在推理后
  从 `sample_output.json` 复算，但复算仍必须调用 `CERMetric` / `WERMetric`。
- 如果 metric 环境缺依赖，必须修复或使用项目已有可用环境，例如
  `PYTHONPATH=src .venv.hostbak/bin/python ...`；不得退回手写 CER/WER。
- `sample_output.json`、`verdict.json` 和 summary 中应记录 metric backend，例如
...
- 验证脚本不得硬编码 `use_gpu=1`、`device="cuda"` 或等价参数后直接进入推理。
  对 FireRedASR 这类上游 API，`use_gpu=1` 会在无 CUDA 节点上直接触发
  `.cuda()` 并失败。必须先检查 `torch.cuda.is_available()`，无 CUDA 时使用
  CPU fallback，并在 verdict / gpu_status 中明确记录
  `local_uv_gpu_available=false`。
...
```text
docker.v2.aispeech.com/sjtu/sjtu_yukai-dujunhao-sure_qwen__qwen3-asr-1.7b:v1.1
docker.v2.aispeech.com/sjtu/sjtu_yukai-dujunhao-sure_gilgameshwind__x-asr-zh-en:v1.0
```

不要用 `sure_asr_<name>`、`sure_x_asr_zh_en` 这类任务前缀或人工简称命名。
ASR 目录和镜像必须从 `weights.repo_id` 推导。
...
如果本地 local uv 或本地 Docker 在 2080Ti/11GB 等小显存卡上 OOM，不能把 OOM
当作 ASR onboarding 闭环终点。必须继续提交到 VC/超算更大显存 GPU 队列跑
同一个 Docker validate，并在 verdict 中记录 `local_gpu_oom` 与
`vc_gpu_passed` / `vc_gpu_failed` 的区别。FireRedASR-LLM-L 就是这种情况：
本地 2080Ti 能加载但推理 OOM，下一步必须是 VC GPU 验证。
...
- [ ] WER/CER 由 `CERMetric` / `WERMetric` 计算，并在 artifact 中记录
      `backend=sure_eval.evaluation.tasks.asr.metrics`。
- [ ] 本地 uv GPU 通过，或失败原因已结构化记录。
- [ ] Docker GPU 通过。
- [ ] 如果本地 GPU/Docker OOM，已提交 VC/超算 GPU 验证并记录 job/log/verdict。
