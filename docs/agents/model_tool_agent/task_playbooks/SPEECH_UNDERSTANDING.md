...
`moonshotai__Kimi-Audio-7B-Instruct` / Kimi-Audio-7B-Instruct 的接入经验。适用任务：
...
- 多任务 metric 脚本索引：
  ...
  - 汇总命名空间：`src/sure_eval/evaluation/tasks/slu/ and src/sure_eval/evaluation/tasks/classification/`
...
- SA-ASR/SD 等 annotation 任务要额外检查 conversion profile。若模型输出不是 metric
  ...
  包级 `src/sure_eval/evaluation/conversion/{task_slug}__{metric_slug}/` 只放可复用代表 profile。
  同时确认 `conversion_trace` 出现在报告中。
...
S2TT metric 输入必须按 metric 明确准备，不要假设所有 metric 都只需要
`ref/hyp`：
...
对应权重和 tokenizer/source 也放在各自 scoring 子目录：

```text
scoring/xcomet_xl/checkpoints/xcomet_xl/
scoring/xcomet_xl/checkpoints/xlm_roberta_xl/
scoring/bleurt_20/checkpoints/bleurt_20/saved_model/
scoring/bleurt_20/checkpoints/bleurt_source/
```

不要把长期使用的 metric cache 或模型权重放到 `/tmp`。
...
decode artifact，例如 `!在此次申办冬奥会的过程中`、`!C`。不能先把它当普通文本
normalization 处理，必须先拿 token 级证据：
...
```text
docs/agents/model_tool_agent/memory/bad_cases/kimi_audio_leading_bang_token.md
```

## 6. Evaluation

正式评估应优先复用新 task routes：

- ASR: 记录 CER/WER，至少 CER。
- S2TT: BLEU / chrF。
- SER: `tasks/classification` + `scoring/classify`。
- SLU: `tasks/slu`，先走 `normalization/prompt_norm`，再走 `scoring/classify`。
- GR: `tasks/classification` + `scoring/classify`。

`SUREEvaluator` 和 `sure_eval.evaluation.tasks.classification.metrics.AccuracyMetric`
仍保留为兼容入口，但新报告中必须能看到新 pipeline trace。

第一阶段 success 只要求链路完整；metrics 是模型表现，不是接入是否成功的唯一条件。

`validate_multitask.py` 可以直接在推理完成后调用正式 metric 类并落盘
`speech_understanding_metric_report.json`：

- ASR: `sure_eval.evaluation.tasks.asr.metrics.CERMetric`
- S2TT: `sure_eval.evaluation.tasks.s2tt.metrics.BLEUMetric`
...
如果选择先推理、后评测，也必须用同一批 `ref_*.txt` / `hyp_*.txt` 生成上述 report。

正式多任务指标必须落成独立 report，不能只依赖 `validate_multitask.py` 内部的临时
metric 字段。已建立的通用入口：
...
该 runner 从已有 ref/hyp 文件计算指标，不重新跑模型推理：

```bash
env -u HTTP_PROXY -u HTTPS_PROXY -u http_proxy -u https_proxy -u ALL_PROXY -u all_proxy \
PYTHONPATH=src .venv.hostbak/bin/python scripts/run_speech_understanding_metric_pipeline.py \
  --model moonshotai__Kimi-Audio-7B-Instruct \
  --artifacts-dir src/sure_eval/models/moonshotai__Kimi-Audio-7B-Instruct/artifacts \
  --tasks ASR,S2TT,SER,SLU,GR \
  --asr-language zh \
  --s2tt-language zh \
  --output src/sure_eval/models/moonshotai__Kimi-Audio-7B-Instruct/artifacts/speech_understanding_metric_report.json
```
...
- `ok: true`
- `errors: []`
- ASR backend 是 `sure_eval.evaluation.tasks.asr.metrics.CERMetric` 或 `WERMetric`
- S2TT backend 是 `sure_eval.evaluation.tasks.s2tt.metrics.BLEUMetric`
- SER / GR backend 是 `sure_eval.evaluation.tasks.classification.pipeline.evaluate_classification_files`
- SLU backend 是 `sure_eval.evaluation.tasks.slu.pipeline.evaluate_slu_files`
- SLU pipeline trace 包含 `normalization/prompt_norm` 和 `scoring/classify`

Kimi-Audio 已验证 report：

```text
src/sure_eval/models/moonshotai__Kimi-Audio-7B-Instruct/artifacts/speech_understanding_metric_report.json
```

如果 `S2TT` 报 `No module named 'sacrebleu'`，这是评测环境依赖缺失，不是模型推理失败。
按 uv playbook 使用清华源给当前 SURE 环境补依赖后重跑同一个 metric runner。
cache 必须放在 SacreBLEU scoring 目录下，不要依赖 `/tmp`：

```bash
env UV_CACHE_DIR=src/sure_eval/evaluation/nodes/scoring/sacrebleu/.cache/uv uv pip install \
...
```text
docker.v2.aispeech.com/sjtu/sjtu_yukai-dujunhao-sure_moonshotai__kimi-audio-7b-instruct:v1.0
```
...
KIMI_AUDIO_DEVICE_MAP=auto \
KIMI_AUDIO_MAX_MEMORY=0:4500MiB,1:4500MiB,cpu:80GiB \
src/sure_eval/models/moonshotai__Kimi-Audio-7B-Instruct/docker_validate_multitask.sh
```

绑定本地 GPU：

```bash
DOCKER_GPUS=device=0 src/sure_eval/models/moonshotai__Kimi-Audio-7B-Instruct/docker_validate_multitask.sh
```

调试单任务：

```bash
KIMI_AUDIO_VALIDATE_TASKS=SLU src/sure_eval/models/moonshotai__Kimi-Audio-7B-Instruct/docker_validate_multitask.sh
```

## 9. VC / 集群运行

完整五任务建议通过 VC 独占资源跑。当前 re-onboarding 已验证 `pdgpu-3090-data`
可以完成 Kimi-Audio 五任务全量 smoke；A10 也可作为备选，但可能排队。

re-onboarding 目录示例：

```bash
env -u HTTP_PROXY -u HTTPS_PROXY -u http_proxy -u https_proxy -u ALL_PROXY -u all_proxy \
VC_PARTITION=pdgpu-3090-data \
VC_JOB_NAME=reonboard-kimi-full-3090-data \
KIMI_AUDIO_VALIDATE_TASKS=ASR,S2TT,SER,SLU,GR \
KIMI_AUDIO_DEBUG_TOKENS_PATH=/hpc_stor03/sjtu_home/junhao.du/sure-eval-sandbox/src/sure_eval/models_reonboard/runs/asr_kimi_audio/artifacts/kimi_token_debug_full_3090_data.jsonl \
src/sure_eval/models_reonboard/runs/asr_kimi_audio/vc_validate.sh
```

原模型目录 handoff 中 A10 示例：

```bash
env -u HTTP_PROXY -u HTTPS_PROXY -u http_proxy -u https_proxy -u ALL_PROXY -u all_proxy \
vc submit \
  -p pdgpu-a10 \
  -i docker.v2.aispeech.com/sjtu/sjtu_yukai-dujunhao-sure_moonshotai__kimi-audio-7b-instruct:v1.0 \
  -j kimi-audio-five-task \
  -n 1 -c 8 -m 32G -g 1 \
  -pj sjtu \
  -d /hpc_stor03/sjtu_home/junhao.du/sure-eval-sandbox/src/sure_eval/models/moonshotai__Kimi-Audio-7B-Instruct \
  -e PYTHONPATH=/hpc_stor03/sjtu_home/junhao.du/sure-eval-sandbox/src \
     KIMI_AUDIO_VALIDATE_TASKS=ASR,S2TT,SER,SLU,GR \
     KIMI_AUDIO_LOAD_IN_8BIT=0 \
     PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True \
     KIMI_AUDIO_MODEL_PATH=/hpc_stor03/sjtu_home/junhao.du/sure-eval-sandbox/src/sure_eval/models/moonshotai__Kimi-Audio-7B-Instruct/.runtime/modelscope_cache/models/moonshotai/Kimi-Audio-7B-Instruct \
...
artifacts/multitask_sample_output.json
artifacts/speech_understanding_metric_report.json
```

## 10. 已知 Kimi-Audio 五任务结果

re-onboarding 最新已验证完整 VC run：
...
artifacts:
  ...
  src/sure_eval/models_reonboard/runs/asr_kimi_audio/artifacts/speech_understanding_metric_report.json
  src/sure_eval/models_reonboard/runs/asr_kimi_audio/artifacts/kimi_token_debug_full_3090_data.jsonl
```
...
S2TT: COMPLETE, BLEU=13.9676 on 3 samples
...
- [ ] 生成式模型的开头 decode artifact（如孤立 `!` / `！`）已先做 token 级根因定位，并在 detokenize / generation protocol 层修复。
- [ ] 输出 ref/hyp 文件和 `multitask_sample_output.json`。
- [ ] 输出 `speech_understanding_metric_report.json`。
- [ ] 正式 metric report 由 `scripts/run_speech_understanding_metric_pipeline.py`
      生成，并引用 `tasks/asr`、`tasks/s2tt`、`tasks/classification`、
      `tasks/slu` 以及对应 `nodes/*`。
