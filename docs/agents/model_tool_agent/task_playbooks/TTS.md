...
- TTS re-onboarding、model onboarding、Docker validation 产出的 TTS 评测结果
  必须通过 `src/sure_eval/evaluation/tasks/tts` 的正式工具链计算。
- 生成音频的 `validate.py` 可以先做 phase-1 audio contract 检查，但不能把
  “输出音频存在、可解码、不是 prompt copy” 当作 TTS 评测完成。
- 评测入口必须使用 `TTSSample` + `TTSMetricPipeline`，优先使用
  `build_default_tts_metric_pipeline()`；如果某些重模型 provider 缺 checkpoint 或
  资源不可用，必须记录为 `blocked` / `not_available`，不能退回手写相似度、手写
  MOS 或只用文件大小作为 metric。
- 每个模型至少要写出：
  - `artifacts/sample_output.json` 或 `docker_artifacts/sample_output.json`
  - `artifacts/tts_metric_report.json` 或 `docker_artifacts/tts_metric_report.json`
  - `verdict.json` 中的 `tts_evaluation` 字段
- `tts_metric_report.json` 必须记录：
  - `backend: "sure_eval.evaluation.tasks.tts"`
  - `sample.prediction_audio`
  - `sample.reference_text`
  - `sample.reference_audio`
  - metric `results`
  - provider failures/blockers
- 如果只是修正 TTS metric 口径，不允许重新跑 TTS 推理；必须复用已有生成音频
  和 sample_output 中的 prompt / target text，再用 TTS evaluation 工具重算。

### 3.1 正式 TTS metric 复用入口

不要在模型 Docker 的 `validate.py` 里硬装完整 TTS evaluator 依赖，也不要在
node-local metric 环境为空时临时拉大包。当前正式入口是 evaluation CLI，它会先
描述任务 route，再通过 `samples_jsonl` 调用统一 script 层：
...
- 模型自身的 `validate.py` / `docker_validate.sh` 只负责
  import/load/infer/audio contract，并产出 `sample_output.json` 和合成音频。
- 正式 TTS 指标复用 `sure-eval metric describe/run`：
  - 读取已有合成音频；
  - 读取 prompt/reference audio；
  - 读取 target/reference text；
  - 通过 `src/sure_eval/evaluation/tasks/tts` 的 `TTSSample` / `TTSMetricPipeline`
    计算指标；
  - 输出标准 `report.json` 和 `pipeline_description.json`。
...
.venv.hostbak/bin/python scripts/run_tts_metric_pipeline_docker.py \
  --prediction-audio /hpc_stor03/sjtu_home/junhao.du/sure-eval-sandbox/src/sure_eval/models_reonboard/runs/IndexTeam__IndexTTS-2/docker_artifacts/outputs/indextts2_reonboard_smoke.wav \
  --reference-audio /hpc_stor03/sjtu_home/junhao.du/sure-eval-sandbox/src/sure_eval/models_reonboard/runs/IndexTeam__IndexTTS-2/fixture/tts/zh/ZH_B00001_S00000_W000000.mp3 \
  --reference-text '这是一个新的本地语音合成验证样例，用来确认模型会根据输入文本重新生成语音，而不是返回参考音频本身。' \
  --language zh \
  --sample-id indextts2_reonboard_smoke \
  --gpu 0 \
  --device cuda:0 \
  --work-dir /hpc_stor03/sjtu_home/junhao.du/sure-eval-sandbox/src/sure_eval/models_reonboard/runs/IndexTeam__IndexTTS-2/docker_artifacts/tts_metric_parts \
  --output /hpc_stor03/sjtu_home/junhao.du/sure-eval-sandbox/src/sure_eval/models_reonboard/runs/IndexTeam__IndexTTS-2/docker_artifacts/tts_metric_report_local_pipeline.json
```
...
```text
tts_cer = 0.0
sim/wavlm-large = 0.8451941186551428
sim/ecapa-tdnn = 0.9108513774025474
sim/eres2net = 0.88568
sim = 0.8805751653525634
dnsmos = 3.253600018907438
wv-mos = 3.6038668155670166
utmos = 2.6453607082366943
```

注意：

- 这个 wrapper 仍然是通过 `src/sure_eval/evaluation/tasks/tts` 计算指标，不是绕过
  SURE-EVAL metric。
- wrapper 会使用已有评测镜像，例如：
  - `docker.v2.aispeech.com/sjtu/sjtu_yukai-dujunhao-sure_swivid__f5-tts_emilia-zh-en:v1.0`
  - `docker.v2.aispeech.com/sjtu/sjtu_yukai-dujunhao-sure_indexteam__indextts-2:v1.0`
...
- 模型 Docker 内缺 `funasr`、`speechbrain`、`onnxruntime` 等 evaluator 依赖时，
  不要先改模型 Docker；优先复用上述 metric wrapper。
...
Re-onboarding 本地 uv 规则：

- 在 `src/sure_eval/models_reonboard/runs/<model>/` 下验证 TTS 时，`.venv`
  必须是该 run 目录中新建的真实目录，不能软链到原始模型目录的 `.venv`。
- 允许复用原始模型 `.runtime` 权重/cache，但 local uv 的 Python 环境必须重新解析并安装。
- F5-TTS 已验证的新 uv 修复项：`pyarrow<21`，用于规避 `datasets 2.14.x`
  与新版 pyarrow 的 `PyExtensionType` 兼容问题。
- IndexTTS-2 已验证的新 uv 修复项：`torch==2.8.0+cu128`、
  `torchaudio==2.8.0+cu128`、`transformers==4.52.1`、
  `modelscope==1.27.0`、`descript-audiotools==0.7.2`。

模型本地 uv 环境不必承载完整 TTS evaluator 依赖。若 `validate.py` 内部尝试
`build_default_tts_metric_pipeline()` 并因 `funasr`、`structlog` 等评测依赖缺失
而写出 `blocked`，不要因此判定模型推理失败。正式指标仍按 3.1 节使用
`scripts/run_tts_metric_pipeline_docker.py` 评估已生成音频。
...
正式 re-onboarding 闭环中，若需要客观 TTS 指标，应在模型推理完成后运行
`scripts/run_tts_metric_pipeline_docker.py` 或 `.sh`，而不是在模型 Docker 的
`validate.py` 里直接承担所有 metric provider 依赖。
...
- [ ] TTS 评测严格调用 `src/sure_eval/evaluation/tasks/tts`，并写
      `tts_metric_report.json`；任何缺失 provider 都结构化记录为 blocker。
- [ ] 正式指标优先复用 `scripts/run_tts_metric_pipeline_docker.py` / `.sh`，
      输入已有合成音频、reference audio 和 reference text。
