...
and evaluates metrics through the route-backed evaluation script layer.
...
# Map sub-task directory names -> evaluation kwargs for sure_eval.evaluation.scripts.run_task
# Example for ASR:
#   {"asr_zh": {"task": "asr", "language": "zh", "metric": "cer"}}
...
def run_validation(subtask: str) -> dict[str, Any]:
    ...
    # ------------------------------------------------------------------
    # 5. Prepare ref / hyp files for the evaluation script layer
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # 6. Evaluate with the route-backed script layer (via root venv to avoid dep mismatch)
    # ------------------------------------------------------------------
    ...
    if not root_venv_python.exists():
        raise FileNotFoundError(
            ...
            "Cannot run the SURE evaluation script layer."
        )

    eval_output_dir = ARTIFACTS_DIR / "metric_reports" / subtask
    eval_payload = {
        "repo_src": str(REPO_ROOT / "src"),
        "task": eval_kwargs.get("task", TASK_TYPE.lower()),
        "language": language,
        "metric": eval_kwargs.get("metric", "cer" if language.startswith("zh") else "wer"),
    ...
    eval_script = (
        ...
        "report = run_task("
        "payload['task'], ref_file=payload['ref_file'], hyp_file=payload['hyp_file'], "
        "language=payload['language'], metric=payload['metric'], output_dir=payload['output_dir']"
        ");"
        "print(json.dumps({"
        "'score': report.score,"
        ...
        "'report_path': payload['output_dir'] + '/report.json',"
        "'pipeline_description_path': payload['output_dir'] + '/pipeline_description.json'"
        "}, ensure_ascii=False))"
    )

    proc = subprocess.run(
        [str(root_venv_python), "-c", eval_script, json.dumps(eval_payload, ensure_ascii=False)],
        capture_output=True,
        text=True,
    )
    if proc.returncode != 0:
        raise RuntimeError(f"SURE evaluation script layer failed: {proc.stderr.strip()}")
