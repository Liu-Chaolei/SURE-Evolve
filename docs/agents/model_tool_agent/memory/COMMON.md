...
- Keep model changes inside `src/sure_eval/models/<model_dir_slug>/` unless the
  task explicitly requires shared harness changes. `model_dir_slug` must be
  derived from `model.spec.yaml` `weights.repo_id` by replacing `/` with `__`;
  never derive it from `model_name`, task prefixes, or informal aliases.
- Docker image tags must align with the same repo id slug:
  `docker.v2.aispeech.com/sjtu/sjtu_yukai-dujunhao-sure_<image_slug>:vX.Y`,
  where `image_slug = weights.repo_id.replace("/", "__").lower()`.
