...
flowchart TD
  ...
  B -- ASR --> ASR[tasks/asr + nodes/normalization/aispeech_norm + nodes/scoring/wenet_wer]
  B -- S2TT --> S2TT[tasks/s2tt + nodes/scoring/sacrebleu/xcomet_xl/bleurt_20]
  B -- SER / GR --> CLS[tasks/classification + nodes/scoring/classify]
  B -- SLU --> SLU[tasks/slu + nodes/normalization/prompt_norm + nodes/scoring/classify]
  B -- TTS / VC / KWS --> FUT[task route + node-local metric env]
```
