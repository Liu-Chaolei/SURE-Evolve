...
def read_code(value: str, _id: str) -> str:
    ...
    """
    # Try to extract ```python ... ``` or ``` ... ``` code block
    match = re.search(r"```(?:python)?\s*(.*?)\s*```", value, re.DOTALL)
    if match:
        value = match.group(1).strip()
    else:
        # No code block found - check if this looks like valid Python code
        # Filter out common non-code patterns (function tags, XML-like tags, etc.)
        if value.strip().startswith('<') or '<function>' in value or '<tool_call>' in value:
            # This is not Python code, return empty to trigger error
            return "", value

    # Additional filtering: remove <function> tags if they somehow got through
    value = re.sub(r'<function>.*?</function>', '', value, flags=re.DOTALL)
    value = re.sub(r'<function>', '', value)
    value = value.strip()
