...
def align_public_materialization(
    ...
    # Temporary user-authorized bibliography deferral. A raw identifier is not
    # a fabricated title and remains explicitly marked as awaiting verification.
    for paper_id in unresolved:
        reference_by_id[paper_id] = paper_id
    ...
    idea_result['reference_validation'] = {
        'status':'deferred' if unresolved else 'resolved',
        'unresolved_ids':unresolved,
        'warning':'Bibliographic lookup deferred by user; unresolved IDs are not verified citations.' if unresolved else '',
    }
