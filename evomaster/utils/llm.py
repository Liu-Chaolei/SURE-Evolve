...
class OpenAILLM(BaseLLM):
    ...
    def _call(
        ...
        **kwargs: Any,
        ...
        # Use streaming to avoid CDN timeout (HTTP 524) on long generations
        request_params["stream"] = True
        request_params["stream_options"] = {"include_usage": True}

        # Collect streamed chunks
        content_parts: list[str] = []
        tool_calls_delta: dict[int, dict] = {}
        finish_reason = None
        usage_info = None
        model_name = None
        response_id = None
        ...
        for chunk in stream:
            ...
            if delta.tool_calls:
                for tc_delta in delta.tool_calls:
                    idx = tc_delta.index
                    if idx not in tool_calls_delta:
                    ...
                    if tc_delta.function:
                        if tc_delta.function.name:
                            tool_calls_delta[idx]["name"] = tc_delta.function.name
                        if tc_delta.function.arguments:
                            tool_calls_delta[idx]["arguments"] += tc_delta.function.arguments
        ...
        if tool_calls_delta:
            tool_calls = [
                ...
                for tc_data in sorted(tool_calls_delta.values(), key=lambda x: x["id"])
            ]
